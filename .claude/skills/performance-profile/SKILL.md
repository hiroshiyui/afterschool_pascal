...
```sh
time build/bin/pascalc --emit-llvm -O0 /tmp/big.pas -o /dev/null   # front end + codegen only
time build/bin/pascalc --emit-llvm -O2 /tmp/big.pas -o /dev/null   # + the pass pipeline
time build/bin/pascalc -c            -O2 /tmp/big.pas              # + object emission
time build/bin/pascalc               -O2 /tmp/big.pas -o /tmp/big  # + the clang link
```
...
```sh
perf record -g --call-graph dwarf -- build/bin/pascalc -O2 --emit-llvm /tmp/big.pas -o /dev/null
perf report --stdio
```
...
```sh
build/bin/pascalc -O2 bench.pas -o /tmp/bench && perf stat /tmp/bench
```
