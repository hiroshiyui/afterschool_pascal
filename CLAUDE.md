...
Twenty-five gates make that mechanical, and each fails in **both** directions — a
...
questions about a conformance mode, and there is none. Three of the twenty
...
| `variant-check` | ADR-0118's guard, and the corpus | is every node read through the arm its tag selects? (ADR-0223) — §6.5.3.3 makes a wrong-arm read an error and §3.1 lets a processor leave it undetected, which this compiler did in itself: ADR-0118's check existed and was a *dialect* rule, and `compiler.std` said `extended`. ADR-0223 answered that by building the compiler a second way to arm the guards; ADR-0232 removed the modes, so the guards are in whatever this compiler emits and what is left is the sweep — compile the compiler with itself, then read the corpus with the result, which is 952 sources carrying 2935 guards as this is written and moves with both — run the gate, which prints the pair. **The mutation is the argument**: `nkStr`'s `(stAt, stLen)` and `nkVar`'s `(vrAt, vrLen)` share storage, so reading the wrong one yields the right value — 808 ctest cases and 323 scenarios all green, and it caught the mutation on 582 of the 1019 sources the corpus then held. It refuses to pass by asking nothing: fewer than 100 guards is a failure |
