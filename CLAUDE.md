...
| `wcwidth` | `tests/checks/wcwidth_disagreements.txt`, and the C library's own table | does a second implementation give a code point the same width? (ADR-0398, ADR-0399) — AP 6.4.15.13's table is generated from `EastAsianWidth.txt`, and **that property is the one Unicode publishes no conformance file for**, so `unicode-conformance` regenerating the header and diffing it checks the *generator* is stable rather than the table right. `fpc-differential`'s rule: not an authority, so where the two differ the clause decides. **What is catalogued is a cause and not a code point**, and that is ADR-0399 correcting ADR-0398 the same day: a range list is a fact about *one* libc's table, and the first version passed where it was written and failed on macOS and ubuntu:24.04 — every machine has a libc and they differ, where `fpc-differential` may enumerate because Free Pascal is absent or present. The five causes are properties of the code point: two read off the `pas_u_gcb` table the header already carries, two written out as ranges generated from the database, and two named code points. **Spacing marks are a class of their own** and that is CI's second correction — UAX #29 does not put every Mc in SpacingMark, the Myanmar ones being Other, so the break classes alone still failed on macOS. The **jamo** one is the interesting one: the two agree about every text and disagree about every code point, `wcwidth` being per code point where this clause measures *elements*. Ambiguous is 179 **ranges** and not the rule *one against two*, which would have excused a generator that made every Wide code point one cell. **Exact agreement is not the claim and cannot be**: two C libraries are built from different Unicode versions, ubuntu:24.04's predating the release that made U+2630..U+2637 Wide, and which code points a library is behind on is a property of its age. So divergence is **bounded** at 500 uncatalogued code points, and the bound is measured rather than chosen — the smallest systematic error this table could hold is 2307 code points, a version skew is tens. Floors beside it: 100 000 code points compared, because glibc answers -1 outside the locale's charmap and the `C` locale would agree with everything it never looked at, and 300 generated ranges, so neither range class can excuse everything by being empty. What it cannot see is an error *inside* a class the clause declares open — a generator that stopped zeroing marks would have every disagreement explained by `cluster` — and that is a `doc/sop.md` §7 row rather than an oversight. Skips 77 without a C compiler or a UTF-8 locale; `WCWIDTH_REQUIRE` refuses to pass by skipping |
