...
| `predicate-callers` | `Assignable`'s own refusal arms and call sites | does every caller of a shared predicate refuse what the predicate refuses? (ADR-0146) — ADR-0058's sentence, *a permission granted in a shared predicate leaks to every caller*, has cost twice over one permission: the relational operators emitting invalid IR for two slices (ADR-0139), then assignment copying one array's contents over another's, exit 0 (ADR-0143). Both fixes were followed by a probe over the positions someone thought of. This derives the positions from the **source** — 21 of them over 16 routines, against the three types `Assignable` refuses outright — and requires the program to be refused. Moving the slice arm one line down leaves all 625 cases green |
...
- **It is specified, and the specification is enforced.** `tests/spec/` takes
  `@afterschool:<clause>` beside the two standards' tags, and 46 of the spec's
  48 testable clauses are cited by a scenario. The clause table is **generated
