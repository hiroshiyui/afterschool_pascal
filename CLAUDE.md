...
| `seed-portable` | every module under `seed/` | does the committed seed name the machine that made it? (ADR-0347) — ADR-0293 puts a trap's own source path into the emitted module, and `seed/refresh.sh` was handing the compiler an absolute one, so the artefact held a stranger's home directory and `seed_current.sh` could pass only in the directory that generated it. That check asks the whole question and **cannot be a ctest case** — the seed is legitimately stale between releases — so it answered eight releases late, in the tag job. This half holds on every push, and is ADR-0366's first conversion from shell to Python |
