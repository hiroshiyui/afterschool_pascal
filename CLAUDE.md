...
| `runtime-nonposix` | `tests/checks/nonposix_headers.txt` | is the runtime still portable to a target that is **not POSIX**? (ADR-0369) — the row above bounds each unit by the headers it *may* include, which is a claim about this source; this asks whether they are **there**. Every target the tree compiles the runtime for is `*-linux-gnu`, so nothing here noticed the drift. **A floor becomes a list**: a missing header is a fatal error, so compiling `pasrt_posix.c` reports `netdb.h` and stops — each `#include <...>` is probed on its own and seven come back, of seventeen, **ten being present**, so what a port lacks is sockets, the terminal and `posix_spawn` and not the file model. Two claims in both directions, a unit's status and its absent headers, so progress is a commit that edits the catalogue. It reads no diagnostic text — every answer is an exit status. **The target is `wasm32-wasi` since ADR-0382** — it was mingw-w64, and the question was never about Windows. Two flags are part of the toolchain and both are load-bearing: `-mllvm -wasm-enable-sjlj`, without which `<setjmp.h>` refuses and the real blocker hides behind a header a flag would have satisfied; and `-nostdlibinc`, because `/usr/include` is on clang's search path for that target and a header wasi lacks would otherwise be found in **glibc** and reported present. The finding is that `pasrt.c` is blocked on `_longjmp` on *both* targets measured, which makes the non-local goto the runtime's one real portability question. Skips 77 without a cross compiler; `NONPOSIX_REQUIRE` refuses to pass by skipping |
