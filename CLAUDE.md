...
tools/release.py --archive build v3.5.1     # what a tag ships; --check <archive> is what the job runs on it (ADR-0296)
...
directory it created for the run**, and a port is asked for rather than
assumed. That was true of a harness's own scratch files and **false of the
program under test** until ADR-0406, which started it in that directory too:
every case shared the invoker's, and `lib_process_execute.pas` — the one case
that names a file relatively, being about what a child can see — raced its own
concurrent copy, one `rm`ing the file the other was opening. Eight failures in
twenty concurrent pairs, and none after.
...
non-zero exit is then required — including a refusal that lands on one of
`foo.components`'s *components*, which the harness bailed on and compared
nothing about until ADR-0413), `foo.warn` (expected *warnings*, for a program
...
own build tree. **A third variable says what *starts* a compiled program** (ADR-0384):
`AFTERSCHOOL_PASCAL_RUNNER` is a command, split on blanks and prepended by
`tests/run_test.py` and `selfhost/irtest.py` — and by nothing else — so a program
built for a target this machine cannot execute is started by a runtime instead.
The runtime's own flags (a WASI `--dir` for the two scratch paths a case is
handed) are part of the command and not of the harness, and the *toolchain* is
never wrapped: a compiler is a program for the machine it runs on whatever it
emits for.
**Two variables add flags and they are not interchangeable**:
...
| `kind-exhaustive` | `tests/checks/partial_cases.txt` | does every dispatch over an enumeration name every constant? (ADR-0124, ADR-0145, ADR-0221, ADR-0229, ADR-0230, ADR-0404) — **in every program here and not only the compiler**, which it was for its whole life until `MenuKey` crashed the shipped editor on two key kinds ADR-0396 had added and the compiler had been able to name all along; five corpora now, and adding one is a line. A site belongs to the **first** corpus that reaches it, `apedit.pas` being imported by two of them and an argument written twice being ADR-0388's own defect; whether a constant is named by *no* case-statement is a question about the **tree**, `crText` being dispatched in the shell and not in the model by design — a constant left off a **case-statement** stops the program (ADR-0018) and is a compiler crash; one left off an **if-chain** takes the trailing `else` in silence, which is why a bare `else` does not excuse a chain where `otherwise` excuses a case. Both halves come from `--dump-dispatch`, so the gate reads no Pascal |
...
| `stale-component` | a component's source, edited between two links | is an object built from an older module-heading refused, and is a method reached by a name? (ADR-0245, ADR-0411, AP 6.13.2) — the first cost a **wrong answer with a zero exit status** and no diagnostic from compiler, driver or linker. It is a harness of its own because **no test case can edit its own source between two compilations**, and it fails in both directions: a comment, a reflow and a method's *body* must still link, where a changed **method heading** must not — AP 6.7.10 puts an implementation in the block and AP 6.7.10.5 makes it reachable, so the heading digest covers the implementations too. Its sixth claim is the one **no ctest case can make**: `run_test.py` hands every translation the same components in the same order, under which a counter agrees with itself, so only a module translated *alone* against a client that read another component first tells a composed name from a counter |
...
| `helper-portability` | `tests/checks/helper_portability.txt` | is ADR-0366's rule still true of this tree? (ADR-0367) — that decision made a helper here Python 3 reaching for the standard library, and thirty-one conversions carried it out, leaving a rule nothing enforced. Three claims, all in both directions: no tracked shell script the catalogue does not name — by extension **or** by `#!` line, which is what sees `tools/pascalcc`; none of the constructs bash 3.2 lacks in the one it does name, `case` inside `$( )` among them, that being a parse and not a name; and no Python helper starting a general-purpose utility to do what the library does, read from the argv shape **and** from a wrapper taking positionals, without which the one call the catalogue allows was invisible to the check written to police it |
| `lib-coverage` | `tests/checks/lib_coverage.txt` | how much of `lib/` does the corpus run? (ADR-0350) — `line-coverage` measures the compiler's three components and nothing else, so the whole of `lib/` — 33 modules today — was measured by nothing; every module *is* imported by a case, which is `procedure-coverage`'s question and not this one. Same attribution trick: **one module instrumented per link**, because `$PASCOV_LINES` is bare line numbers and a program linking six modules otherwise yields six sources' lines in one heap. **And one module's *routines*, since ADR-0416**: a generic's body is emitted in the translation that activates it (AP 6.7.3.5), so a client of `PasContainer` carried seven `Vec` routines with `PasContainer`'s line numbers, which index into the client's source and land on a comment. 50 statements across four modules, and — worse than the inflation — a denominator that moved whenever a module's *length* changed, three times in one day for edits that touched no statement. A routine is the module's when the `; name line` marker opening it points at a line of this source declaring it; the check on that rule is that a dropped name is declared in this source nowhere, which needs no threshold and fails with the name it found. A **generic** module cannot be measured here at all — AP 6.7.3.5 emits its body in the *client's* translation — so it reports a denominator of 0 and the run names it, that meaning *nothing to measure* and never *all covered*. **It runs a case the way the suite runs it** (ADR-0378) — the two scratch paths and the `.in`, which it passed neither of, so seven cases naming a program-parameter stopped at their first statement and three reached nothing past their first `read`: 70 statements the corpus was covering all along. And its **denominators are compared** now, both totals and every module's, ADR-0354's decoration defect having been here too |
...
| `icu-width` | ICU's own reading of `EastAsianWidth.txt` and `UnicodeData.txt` | does a second implementation read the width properties the same way? (ADR-0400) — East_Asian_Width is the one Unicode property here with **no conformance file**, so `unicode-conformance` regenerating the header and diffing it checks the *generator* is stable rather than the table right. The probe applies AP 6.4.15.13's own three rules to ICU's `UCHAR_EAST_ASIAN_WIDTH` and `u_charType`, so what is compared is two independent transcriptions of two files and not two opinions about rendering. **1 112 064 code points, exact agreement, no catalogue** — it catches one code point transcribed wrongly by name, and dropping `Cf` and `Cc` from the zero-width rule as 235 disagreements. It replaced a `wcwidth` gate that was wrong three times in a day (ADR-0398, ADR-0399), and the lesson is in those records: `wcwidth` answers a *width*, which is a policy about what the standard leaves open, names no version, and differs on every machine — a differential against a system library can bound divergence and cannot forbid it. **A missing ICU and a version mismatch are different skips** and `ICU_WIDTH_REQUIRE` covers only the first: a machine whose ICU predates the pinned Unicode cannot answer this however loudly it is asked, which is ADR-0282's abstention. Most images are behind — `debian:trixie` ships ICU 76, which is Unicode 16 — so the comparison is taken in the **one** job already on a digest-pinned `debian:testing`, which greps its own output for `Skipped` because the require variable does not cover a version mismatch |
| `tui-palette` | `tui/apedit.pas`'s roles, `tui/session.pas`'s letters and `tui/apide.pas`'s colours | is every cell role **drawn**, and is every pairing legible? (ADR-0393) — ADR-0389 put a role on a cell and a colour nowhere near the model, so a golden holds the role plane and nothing holds what a person sees. Two defects lived in that gap. `crPrompt` was declared, coloured, lettered and drawn on **no screen** for the life of ADR-0392, which turned a prompt from a bottom line into a box that drew its contents in its own role — fifteen sessions green, and the mutation proves it: put the defect back, regenerate the goldens, and `tui-sessions` still passes while this fails. And two pairings were a **colour on a colour**, which a muted theme flattens — `crFrame` was cyan on blue at 4.8:1 and is the whole body of a drop-down. The rule is structural and the ratio is its evidence: one side of every pair is black or white, never `clDefault`, because what a terminal makes of the eight ANSI colours is its own business. `crText` is exempt and must be, the document being the terminal's own text. Both claims fail in both directions. **Two more claims since ADR-0394**: the twenty-four-bit table beside the eight-colour one, both held to the floor, because a fallback nobody looks at is where an unreadable pair survives; and two regions a person sees at once must differ in their **backgrounds** by 3:1 — legible is not distinguishable, and ADR-0393 shipped the proof, putting the selected menu title at 16.7:1 to read and 1.6:1 from the four beside it so nothing said which menu was open. The co-visible set is **pairs and not sets**, measured: with an extreme on one side of every pairing, no three of the eight ANSI colours are mutually 3:1 apart. It reads no Pascal it did not parse and needs neither a build nor a terminal |
| `tui-terminal` | the editor, driven under a pseudo-terminal | does the shell **emit** what the model decided? (ADR-0402) — `tui-sessions` compares what `tui/apedit.pas` decided and `tui-palette` reads the two colour tables, so a shell painting every row in one flat colour passes both. It drives the real `apide` under a pseudo-terminal and requires every run it writes to be one the model decided, at the same columns, with the same cells, in the colour that role's table gives — over **both** tables, with `COLORTERM` set and unset, because the eight-colour fallback is what a plain `ssh` and the Linux console get and no byte written to one had ever been read. **The expectation is derived and not recorded**: a golden of escape bytes would agree with whoever wrote it, so each script is replayed through `tui/session.pas` as well, whose golden already holds the **run decomposition** `PutRow` is supposed to walk. It is not the binding ADR-0262 declined — the pseudo-terminal is Python's, as `lsp/run.py`'s pipe is. 14 of the 22 sessions can be driven and a floor stops that emptying; the eight that cannot ask the model for what a person at a terminal cannot ask for. It also narrows the raw-mode row to three claims from six, reaching what it reaches because the editor cannot work without it |
...
| `quoted-numbers` (after the suite) | `tests/checks/quoted_numbers.txt`, and every gate's own summary | is a number a document quotes still that gate's answer? (ADR-0379) — **the one shape the documentation procedure says it cannot see**, and saying so did not close it: two audits of `doc/sop.md` §7 found the register clean and six counts around it stale, every one found by running the gate and by nothing else. The evidence is *read* rather than re-measured — each check prints a summary and `ctest` records it — so it costs nothing where re-running `valgrind-corpus` would cost 170 s. **Not a ctest case and cannot be**: during a run that log holds whatever has finished so far, so it is a script run after the suite, and a log older than anything the gates measure makes it skip rather than answer. Both directions: a pattern that stops matching is a sentence reworded away from its row. A record's numbers are out of scope — an ADR says what was true then |
| `markdown-links` | every relative link in a tracked Markdown file | does every link still point at something? (ADR-0377) — **a dead relative link renders exactly like a live one**, so the only way to find one is to follow it and nobody follows the links in a document they are reading for its prose. `doc/adr/README.md`, whose whole job is to be a set of links, pointed at a record renamed the day before, and a sweep found eight more — every one an ADR renamed after something cited it, which ADR-0001 makes permanent. Two claims: the target exists, resolved against the linking file's own directory; and a `#fragment` names a heading there, under GitHub's slug rules, which is how a *reworded heading* is caught. What git tracks is what it sweeps, and an `http://` target is not asked about — a gate that fails when a host is down is one people learn to ignore |
| `wasm32` | `tests/checks/wasm32_known.txt` | does a program built for **WebAssembly** behave? (ADR-0385) — `target32`'s shape one target further on, and its argument: both defects the i386 port found were in neither a layout rule nor a frame. The corpus is built for `wasm32-wasi`, linked against **only the runtime units `nonposix_headers.txt` says compile for it**, and run under a WASI runtime through `AFTERSCHOOL_PASCAL_RUNNER` — so what a program can reach is what a port has got, and one wanting more fails at the link with the symbol named. **571 of 606 answer their golden**; the 35 that do not are one of five causes. It was 519 until ADR-0390, and the 52 that moved were one function: `tmpfile` is ISO C, wasi declares it and does not define it, and the runtime stopped calling it — an exclusive `fopen` and a `remove` are both ISO C and are the whole of what it was doing, so no preprocessor conditional was added. **Thirteen of the 52 were behind a second cause and moved rows rather than passing**, which is why that catalogue is grouped by cause and not counted. It found two things no other oracle here could: a 64 KB default stack that printed stray spaces into the middle of a line rather than failing, and a `SOURCE_DATE_EPOCH` a WASI runtime does not pass on unless told to — both *wrong answers* rather than failures to start. **And it abstains where this clang and this module disagree about a modelled field**, because clang overrides the module's datalayout with its own (ADR-0156): trixie's clang 19 names no `i128:128` for wasm32 and clang 21 does, so an i256 aligns 8 where the compiler computed 16 — a set inside a record laid out two ways, ADR-0028's own defect, and `tests/sets_records.pas` was the one failure in 598 on that image. **wasm32 support requires an LLVM that names `i128` for the target**, which is a property of the toolchain and not of this compiler, and the CI image is `debian:testing` for it. Skips 77 without the sysroot, `llvm-ar`, or a runtime; `WASM32_REQUIRE` refuses to pass by skipping. The engine is **wasmedge**, which is what the committed catalogue was taken with, and `tests/checks/wasi_run.py` is what hands a program the four variables the runtime reads — a WASI program is given no environment, and the set is checked against `runtime/*.c` in both directions |
| `runner-seam` | `AFTERSCHOOL_PASCAL_RUNNER`, and a wrapper that records | does a harness start the program under test through the runner? (ADR-0384) — every harness here executes what it built, which is right until what it built is for another target, and `--target=` admits `wasm32-wasi` since ADR-0383. **A seam with no user is plumbing**: nothing here runs a wasm program yet, so it is exercised by a wrapper that records the command and `exec`s it, which holds the *shape* of the invocation. Four claims — `run_test.py` and `irtest.py` each start the program through it with the two scratch paths, neither starts any part of the **toolchain** through it, and with the variable unset nothing reaches the wrapper at all, which is the floor. Only those two harnesses read it and `doc/sop.md` §7 says why |
...
| `release-archive` | `tools/release.py`, and the archive it writes | does the script a tag runs still produce an archive that checks out? (ADR-0296) — the tag job is the third piece of shell here that runs only at a tag, after two that failed for want of anywhere to be exercised first (ADR-0282) |
| `runtime-isoc` | `tests/checks/nonstandard_c.txt` | how far is the runtime from ISO C, and what does it compile *conditionally*? (ADR-0161, ADR-0186) — it is the only C here and the whole of what a port has to satisfy. `runtime/pasrt.c` carries a short catalogue of POSIX **functions**, proved complete by stripping every non-ISO `#include` from a copy before the strict compile. That mechanism **cannot work for a type**, which is why `runtime/pasrt_posix.c` is bounded by its **headers** instead and required to hold nothing but `pasx_` — so a system without those headers loses library routines and **not the language**. Skips 77 on a C library that declares POSIX anyway |
| `runtime-nonposix` | `tests/checks/nonposix_headers.txt` | is the runtime still portable to a target that is **not POSIX**? (ADR-0369) — the row above bounds each unit by the headers it *may* include, which is a claim about this source; this asks whether they are **there**. Every target the tree compiles the runtime for is `*-linux-gnu`, so nothing here noticed the drift. **A floor becomes a list**: a missing header is a fatal error, so compiling `pasrt_posix.c` reports `netdb.h` and stops — each `#include <...>` is probed on its own and **five** come back, of seventeen, so what a port lacks is name resolution, the terminal and `posix_spawn` and not the file model — and not the sockets either, wasi having an interface of its own. Two claims in both directions, a unit's status and its absent headers, so progress is a commit that edits the catalogue. It reads no diagnostic text — every answer is an exit status. **The target is `wasm32-wasi` since ADR-0382** — it was mingw-w64, and the question was never about Windows. Three flags are part of the toolchain and every one stops a shallower answer standing in for the real one: `-mllvm -wasm-enable-sjlj`, without which `<setjmp.h>` refuses and the real blocker hides behind a header a flag would have satisfied; `-nostdlibinc`, because `/usr/include` is on clang's search path for that target and a header wasi lacks would otherwise be found in **glibc** and reported present; and `-D_WASI_STRICT_PTHREAD`, because two wasi-libc sysroots disagree about whether to *declare* a thread on a target that cannot create one — trixie's refuses and a newer one does not, so `pasrt_task.c` was catalogued `compiles` here and reported blocked on CI, one row that cannot be true of both. **Three units of five compile** — `pasrt.c`, which is the language itself, `pasrt_unicode.c`, and `pasrt_file.c` since ADR-0405 cut the file model out of the POSIX unit — and the other two are blocked by two different kinds of thing: `pasrt_posix.c` by headers, and `pasrt_task.c` by the target having no threads at all, which is the whole task facility and not a header. `pasrt.c` was catalogued blocked on `_longjmp` for a day and that was the gate's own `-std=c11` against the build's `-std=gnu11` (ADR-0383) — **the flag a gate compiles with is part of what it claims**. Skips 77 without a cross compiler; `NONPOSIX_REQUIRE` refuses to pass by skipping |
...
| `target-layout` | the frame types the compiler emits, and the targets `--target=` admits | does every admitted target lay a frame out the way the compiler thinks? (ADR-0157, ADR-0325) — **two claims since a target that is not LP64 was admitted**, because *do the targets agree with each other* is one a 32-bit target falsifies by existing. Targets whose layout arithmetic is identical must lay every frame out identically, which is the old comparison over a **class the run derives** — it was a class per word size until wasm32 was admitted (ADR-0383), which is ILP32 like i386 and aligns an i64 to eight where i386 aligns one to four, so the word size put two targets together and 337 correct offsets were reported as a divergence; and the compiler's own size and alignment must equal LLVM's, asked of **every** target over six record shapes through `--dump-layout`. That second half is what caught the alignment itself — four wrong numbers on the first run wasm32 was admitted, and a wrong alignment costs no diagnostic anywhere. The target list comes from the compiler's own `--target=` refusal, so a seventh is compared without the gate being edited — and one was: `wasm64-wasi` (ADR-0386) landed in a class with x86-64, aarch64 and both Darwins and matched every frame offset, with nothing here edited to admit it |
...
- **Ask the symbol, not the syntax.** A qualified name against a field
  ...
  (ADR-0044, ADR-0053, ADR-0066, ADR-0071, ADR-0087). **A method call is the
  sixth and it splits three ways** (ADR-0410): `p.M(a)` is §6.11.3's qualified
  name and Sema decides it by what `p` denotes, `q^.M(a)` is a variable-access
...
- **Nothing that is two words may depend on how a struct is passed.** A
  ...
  **A trait object is two words and is not a seventh** (ADR-0409, AP Annex
  E.13): `tyDyn` sits beside `tyProc` and `tySlice` in `LlSize` and `LlAlign`
  because it is two pointers wide, and for no other reason — it is a heap
  variable travelling by *address* like a record, so nothing about it depends
  on a calling convention. ADR-0408 wrote it into this list before there was a
  value to pass; don't put it back.
...
- **Refusal by construction beats an enumerated list of what is forbidden —
  where the default is to refuse.** A restricted type refuses arithmetic
  through the diagnostic arithmetic already had; a discriminant-selected
  variant has no designator to attribute a value to. Where a check would have
  gone, the code says why it is not there. **The condition in that first
  sentence is load-bearing and cost an increment to learn** (ADR-0408): a type
  answering false to all 41 predicates was expected to be refused everywhere
  and was instead treated as an ordinary small value, because a variable, a
  field, an array element and a value parameter all *permit* by default and a
  predicate is what would have refused. Ask which way the position leans
  before relying on this. Where it leans the right way it is still the best
  instrument here: ADR-0409 admitted the trait object by giving it the file's
  own answers — `IsMemory` true, `IsStructured` false — which moved
  one cell of a 42 × 22 predicate table and nothing else.
...
- **It reserves no word-symbol, and that is a decision** (ADR-0140). A dialect
  ...
  type-name (ADR-0181), `dyn T` which is that same juxtaposition one token
  over — an identifier followed by an identifier, in a type-denoter position
  (ADR-0408), and `x.M(a)` where a complete variable-access is followed by
  `(`, which no program can write because there is no procedural type
  (ADR-0410). The test for a new spelling is whether a conforming
...
`_longjmp` directly — and **wasi-libc declares it**, behind `_XOPEN_SOURCE`,
so `pasrt.c` compiles for `wasm32-wasi` (ADR-0383). It was catalogued blocked
there for a day because the gate compiled with `-std=c11` where the build uses
`-std=gnu11`; the name is XSI rather than base POSIX, which is why
`_POSIX_C_SOURCE` alone does not reach it.
...
- **`tests/dumps/` is their corpus, and it exists because they had none.** Until
  ...
  output, so it has its own harness (`tests/dumps/run.py`); every case under
...
- **`_setjmp` is the only foreign function the emitted module names**, and one
  ...
  target takes the buffer alone, which was *because* they were all POSIX
  (ADR-0380) until wasm32 was admitted and turned out to agree (ADR-0383);
  wasm64 has no sysroot, so the gate abstains for it and says so (ADR-0386) —
  and it was two:
  ...
  call and its `declare` against each target's real header, so a seventh target
  whose C library disagrees fails there rather than miscompiling in silence.
  The sixth needed the gate handed its sysroot and its SjLj flag, without
  which it reported *no headers here* and was not compared at all.
