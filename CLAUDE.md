...
| `wcwidth` | `tests/checks/wcwidth_disagreements.txt`, and the C library's own table | does a second implementation give a code point the same width? (ADR-0398) — AP 6.4.15.13's table is generated from `EastAsianWidth.txt`, and **that property is the one Unicode publishes no conformance file for**, so `unicode-conformance` regenerating the header and diffing it checks the *generator* is stable rather than the table right. `fpc-differential`'s rule: not an authority, so where the two differ the clause decides and the disagreement is catalogued with which way and why — sixteen ranges in four causes, both directions. The **jamo** rows are the interesting ones: the two agree about every text and disagree about every code point, `wcwidth` being per code point where this clause measures *elements*, so a Hangul syllable is one element of two cells there and 2 + 0 here. A floor of 100 000 code points compared, because glibc answers -1 outside the locale's charmap and the `C` locale would agree with everything it never looked at. Skips 77 without a C compiler or a UTF-8 locale; `WCWIDTH_REQUIRE` refuses to pass by skipping |
