...
| `lib/dialect/pastoml.pas` | A **TOML v1.0.0 document** read, navigated, built and written back — `TomlParse` and `TomlParseChars` answering a `TomlResult`, with `TomlPositionOf` turning the byte it stopped at into a line and a column, which is what a configuration file's reader reports. `TomlKindOf`, `TomlCount`, `TomlAt`, `TomlKeyAt`, `TomlMember` and `TomlPath` for a dotted lookup; `TomlIntegerOr`, `TomlFloatOr`, `TomlBooleanOr`, `TomlStampOr`, `TomlTextInto`; eight `TomlNew*` constructors with `TomlAppend`, `TomlPut` and `TomlTextAdd`; and `TomlRender`. **Not a subset**: bare, quoted and dotted keys, all four string forms, four integer bases with underscores, floats with `inf` and `nan`, all four date-time forms, arrays, inline tables, tables and arrays of tables — and every redefinition the format forbids is refused with a position. A **date-time is §6.4.3.4's own `TimeStamp`** with a nanosecond and a UTC offset beside it, so the calendar rules are `PasTime`'s and are spelled once; an **integer is `integer`**, narrower than TOML's, and one outside -maxint..maxint is `errRange` rather than a wrapped number. A string value is bytes and unbounded, for `PasJson`'s reason: normalising somebody's file would edit it |
