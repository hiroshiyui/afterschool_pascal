...
| `lib/dialect/pasnet.pas` | A TCP connection, as a **handle** and a line at a time, and **its routines are methods of its type** (AP 6.7.10, ADR-0411): 10 exported names where fourteen were, with `Socket` carrying `WriteText`, `WriteLine`, `ReadLine` and `Close` — the `Net` prefix each of the four had went with the one scope §6.11.2 put it there for, a method being selected from the type of its receiver instead (ADR-0412). What stays exported is what a caller has to name before there is a receiver to select from: the types, and `NetConnect`, `NetListen`, `NetAccept` and `NetService`, which answer for a socket's *identity in the system* rather than for the stream of bytes through it. **`NetWait` is a fifth for a reason of the construct rather than the rule**: its receiver would be a `SocketList`, and a schema (§6.4.8) is not a type and can carry no routines of its own. A descriptor is an integer and an integer is numeric, so a program holding one could add to it and close it twice; the runtime owns the socket and a program holds something that closes itself. **Both ends of every call are strings** — a host and a *service*, which is a name (`http`) or a number written out — so nothing names an address family or a port, `getaddrinfo` decides, and a caller gets IPv6 without asking. Ask to listen on service `'0'` and `NetService` reports which port you were given, which is how a program talks to itself. Reading is by line because a socket delivers whatever arrived. **`NetWait` says which of a list of sockets can be served without blocking**, and a listening socket in that list is ready when a connection has arrived — so one thread of control serves many clients, and a server is a loop over `NetWait`, `NetAccept` and `c.ReadLine`. The list is a `SocketList(n)` and a closed client is a hole in it, needing no compaction |
