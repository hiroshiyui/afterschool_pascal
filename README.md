...
| module | what it is |
| --- | --- |
| `lib/dialect/paserror.pas` | `ErrorCode` — six categories — with `ErrorText` and `Failed`; `Fallible(T)`, the one result type every module's is a production of, and `ValueOr(r, whenBad)`, the one accessor over all of them, its type inferred at the call (ADR-0297) |
