...
| `lib/dialect/pashttps.pas` | **HTTP/1.1 over TLS** — `Send`, `Receive`, `Exchange` over a `PasTls.Connection`, and nothing else. It is `PasHttp`'s grammar and `PasTls`'s transport: twenty-four lines, no parser of its own, and it binds nothing. Everything a caller builds and reads is `PasHttp`'s and does not change because the octets went through a TLS session, so moving a program from one to the other changes the type of one variable and the qualifier on three calls |
