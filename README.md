...
assumed. **570 of the 605 corpus programs compile, link and answer their
...
**570 of the 605 programs in this corpus run as WebAssembly** (ADR-0385) --
...
| `lib/dialect/pasprocess.pas` | Two ways to run a program. **`Execute`** and the four beside it — `ExecuteInto`, `ExecuteBoth`, `ExecuteLines`, `ExecuteToFile` — take an `ArgV` built with `NewArgs` and `AddArg`, carry the words as words through `posix_spawnp`, and no shell reads them, so an argument holding a space or an apostrophe is an argument; `Deadline(v, seconds)` kills a run that outlasts it (ADR-0362, ADR-0363). Prefer it wherever any part of the command came from outside the program. **`Run`** — a command line through the shell, answering its exit code or `errIO` — `Capture` and `CaptureLines`, its output into a string or onto a `StrVec` with the code beside it, are the right answer for a pipeline or a redirection a person wrote. And `ExitCode`, `Sleep`, `Seconds`, `CpuSeconds` and `ProcessId`. `Run` flushes the program's own output first, so what was written before the command comes out before it. `ProcessId` is `getpid`: a number no other **live** program has, which is what a program building a temporary file name has to build it from |
...
begin
  IVecNew(v, 4);
  IVecPush(v, 10);         { may reallocate, so `v` is passed by reference }
  writeln(IVecGet(v, 1):1);
  IVecFree(v)
end.
```
...
Beside that: 937 cases under `ctest`, the compiler compiled with itself to a
