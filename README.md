...
non-decimal literals, `pow` and `**`, `and_then`/`or_else`, `protected`
parameters, initial-state specifiers (`value`), binding (`bind`/`unbind`),
direct-access files, and complex numbers. A word-symbol is reserved only when
the feature needing it lands, so until the list above is complete
`--std=extended` accepts some programs a conforming processor would reject.
