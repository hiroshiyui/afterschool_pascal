...
| `runtime/pasrt_posix.c` | what the operating system is asked to **do** — a process, a socket, a terminal, a new directory — bounded by its headers rather than its names (ADR-0186, ADR-0405); everything in it is `pasx_` |
| `runtime/pasrt_file.c` | what it is **asked about** — a file's size and kind, a directory's entries, whether a descriptor has anything to read — the half of the POSIX unit that compiles for a target with a file system and no processes (ADR-0405) |
...
| `runtime/pasrt_unicode_data.h` | generated by `runtime/unicode/generate.py` from the pinned Unicode Character Database, and committed. The database itself is fetched by `runtime/unicode/fetch.py` into a gitignored directory |
| `tools/pascalcc` | compile *and link*, which the compiler cannot do for itself |
| `tools/apconfig.pas` | the project-file reader `pascalcc build` calls: TOML in, `key=value` out (ADR-0361) |
...
identical, byte for byte, checked under `ctest` by `selfhost/irtest.py`.
...
```sh
selfhost/irtest.py build/bin/pascalc-seed   # also runs under ctest
```
...
| `wasi-libc`, `libclang-rt-dev-wasm32` | a target that is **not POSIX**, which is the only way to ask whether the runtime's headers are there rather than merely permitted (ADR-0369). The clang already installed supplies the target; these supply the sysroot, and **the sysroot is what the skip turns on** — `clang` is on every machine that builds this compiler, so a `which` cannot tell one that can be asked from one that cannot, and the gate compiles a `<stdio.h>` probe to find out. It was mingw-w64 until ADR-0382 — the question was never about Windows but about whether `runtime/*.c` still depends only on what each unit may depend on. The answers are pinned to one toolchain on purpose: two mingw-w64 versions once disagreed about a name | `runtime-nonposix`, `setjmp-arity` | `NONPOSIX_REQUIRE`, `SETJMP_ARITY_REQUIRE` |
| `wasmedge`, `llvm-ar`, `lld` | the corpus **run** as WebAssembly (ADR-0385). The sysroot row above is what compiles for the target; these are what archive it, link it and run it. `wasmedge` is what the committed catalogue was taken with and `APASCAL_WASM_ENGINE` names another (`wazero` gave the same answer as wasmedge but for two cases when the catalogue stood at 519). **The image is `debian:testing` and not trixie**, because clang overrides the module's datalayout with its own (ADR-0156) and trixie's clang 19 names no `i128:128` for wasm32 — so wasm32 support requires an LLVM that does, and the gate abstains where it does not. It is **pinned by digest**, `testing` being a rolling target and this the one job here that does not name a stable release: the committed numbers are that image's, and moving it is a commit that runs the job and says whether they moved; `tests/checks/wasi_run.py` is what gives a program the environment its case built, a WASI program being handed none. It does not implement the exception-handling proposal, which the non-local goto lowers through on this target, so four corpus cases are catalogued as *this engine cannot run it* — and only one of them is *about* a non-local goto, one `@_setjmp` anywhere in a module being enough for the whole module to be refused | `wasm32` | `WASM32_REQUIRE` |
...
`tests/*.pas` case gets that for free, `tests/run_test.py` and
...
`tests/run_test.py` compiles, runs, and diffs. Source paths are rewritten to
...
* **`tests/dumps/`** compares what the *compiler* writes to standard output
  under `--dump-tokens`, `--dump-trivia`, `--dump-ast`, `--dump-sema`,
  `--dump-all`, `--dump-layout`, `--dump-predicates`, `--dump-symbols`,
...
* **`tui/sessions/`** is a *screen* rather than a program (ADR-0381): a script
  ...
  **A session is read a second time by `tui-terminal`** (ADR-0402), which
  replays the drivable ones against the real editor under a pseudo-terminal
  and requires the runs it writes to match the golden's own run decomposition
  in the colour each role's table gives. A script it cannot drive is skipped
  and a floor stops that emptying: `say`, `fault`, and a `push`, `name` or
  `size` after the first keystroke are all things a person at a terminal
  cannot ask the model for.
* **Two sidecars name a *search* rather than a file** (ADR-0244).
  ...
  `tests/run_test.py`, `selfhost/irtest.py` and `tests/checks/coverage.py` all
