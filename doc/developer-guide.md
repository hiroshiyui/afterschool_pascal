...
| `mingw-w64` | a target that is **not POSIX**, which is the only way to ask whether the runtime's headers are there rather than merely permitted (ADR-0369), and the one admitted target whose `_setjmp` takes a second argument (ADR-0371). The answers are pinned to one toolchain on purpose: two mingw-w64 versions disagreed about a name | `runtime-nonposix`, `setjmp-arity` | `NONPOSIX_REQUIRE`, `SETJMP_ARITY_REQUIRE` |
