...
| `tests/checks/` | the gates and their catalogues — which diagnostics, procedures and statements the corpus reaches, whether the two front ends still agree, whether a lowering changed without its model, whether a second backend builds the same compiler, and the two rules no `tests/` case can express: that a discarded base still yields a range (`model-drift-base`) and that program-components translated under different `--std` do not link (`mixed-mode-link`, with its own two-component corpus in `tests/checks/mixedmode/`). `foreign-reserved` is the sixth catalogue and reads no file: its list lives in the compiler, in `ReservedForeignName`, and the check compares it against the `declare` and `define` literals the emitter writes (ADR-0121). `kind-exhaustive` is the seventh: it takes the twelve enumerations as the compiler declares them and requires every `case … of` over one to name every constant, because a case-statement with no matching label stops the program and a missing arm is therefore a crash no golden can hold (ADR-0124). Its catalogue, `partial_cases.txt`, holds the 23 that name a subset on purpose and records **how many of how many**, so a constant added to an enumeration fails every partial case over it (ADR-0145). `buffer-headroom` is the eighth and reads the compiler's own `tokMax` against the tokens its own source needs — a watch on a bound rather than a claim, because the array that has to hold this source is the *seed's* and raising the constant here does not raise the one that matters (ADR-0126). `dialect-containment` is the ninth and its corpus is another suite: every case under `tests/extended/` compiled a second time under `--std=afterschool` and required to behave identically, which is ADR-0117's claim witnessed by 228 programs instead of by one (ADR-0138). `reserved-words` is the tenth and reads the lexer's own keyword table, requiring the dialect to reserve exactly what Extended Pascal reserves — the one addition that cannot be made compatibly, and the one the corpus sweep can miss entirely (ADR-0140). `predicate-callers` is the eleventh and is the only one whose corpus is *generated*: it reads `Assignable`'s own unconditional refusal arms for the types and its call sites for the positions, builds a program for each pair, and requires the compiler to refuse it — a sweep over the positions the source contains rather than the ones someone remembered, which is what two probes after two defects had been (ADR-0146). The twelfth and thirteenth are about targets rather than about the language: `target-sizes` compiles `runtime/pasrt.c` itself for every target a cross compiler is installed for, because the two `_Static_assert`s that check `PAS_FILE_SIZE` and `PAS_JUMP_SIZE` live in that file and a copy of the struct here would be a copy free to drift (ADR-0155); `target-layout` reads the `%frameN` type definitions out of what the built compiler emits, folds them to a size and an offset apiece under each target `--target=` admits, and requires the numbers to agree — which is the assumption `LlSize` and `LlAlign` make by answering with one number for every target, and which nothing checked until ADR-0157. `clause-citations` is the fourteenth and reads no compiler at all: every clause number written anywhere in the tree must name a clause of ISO 7185, ISO/IEC 10206:1991 or the dialect specification, its catalogue `nonexistent_clauses.txt` carrying the arguments for the two that do not — one of them a dangling cross-reference in ISO 7185 itself (ADR-0164). `heap-balance` is the fifteenth and is the only one that reads no *output*: the runtime tallies what `pas_new` created against what `pas_dispose` gave back and writes the balance at exit, and this compares every heap-using case against `heap_balance.txt`. Every other gate here compares what a program printed, and a leak prints nothing — making `dispose` free nothing leaves the whole suite and the whole spec suite green and moves nineteen balances. A nonzero balance is not a defect, since no standard obliges a program to dispose what it created, so it is a catalogue failing in both directions rather than a rule (ADR-0183) |
