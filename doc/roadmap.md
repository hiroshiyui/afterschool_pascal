...
**The table below is now struck through end to end**, which makes it a record
rather than a queue. What replaced it is one entry and it is smaller and
sharper than anything here was: `PasHttp` writes to a `PasNet.Socket`, so it
speaks plain HTTP only, and making it speak HTTPS means splitting its grammar
from its transport rather than importing `PasTls` — which would make every
program using HTTP link OpenSSL. That is a refactor of one module with no
question in it that anybody has to answer first.

**Thirty modules exist** — eight conforming and twenty-two dialect, listed
