...
five**: not one needed a language feature. The sixth is TLS, and it turned out
not to be a module at all — see its row. The claim that did *not* hold is the
hash row's reason, which named a constraint the dialect lacks; it needed none
(ADR-0260), and the probe that says so took ten minutes against an estimate
that had stood for months.
...
| ~~**HTTP**~~, and **TLS is declined** | **HTTP done** — `lib/dialect/pashttp.pas` over `PasNet`, and `tests/dialect/lib_http.pas` drives it against a server the test program itself is | This row was right that "HTTP is a module over what exists": request forming with header and body validation, status line and field parsing, case-insensitive lookup, `Content-Length` and chunked framing, every overflow reported as `errFull` and never truncated. Two limits it **states** rather than leaves to be found: a body is *lines*, `PasNet.ReadLine` being the only reader the library has and a CR before an LF not being recoverable without new C; and chunked over CRLF data is reported as `errSyntax` rather than answered wrongly. Redirects are not followed — the hop count, the POST→GET rewrite and whether `Authorization` survives a cross-origin hop are policy a module cannot be right about. **TLS is not deferred for effort and is not a module at all**: it means linking a third-party C library, and *nothing in this repository links anything* — ADR-0085's whole point is that the compiler emits text and `clang` does the rest, and `libpasrt.a` is ISO C plus a catalogued handful of POSIX. Binding OpenSSL would put `-lssl -lcrypto` behind every program that links the runtime, or split the runtime into a part that is always there and a part that is not. That is a decision about what this project *depends on*, and it is the first one here that cannot be answered by writing a probe — so it is written down rather than taken. A program needing HTTPS today shells out through `PasProcess` |
