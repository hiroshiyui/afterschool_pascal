...
| A daily program wants | Why it waits |
| --- | --- |
| **to write a container once** | `PasVector` holds integers, `PasStrVec` strings, `PasList` strings, and `PasMap` maps a string to an integer — four modules where one would do, and the way to have a fifth for another element type is to copy a file. **Both halves of the language change have landed**: ADR-0209 made a container's *storage* writable once (`Vec(T: type; cap: integer)`) and ADR-0211 made the routine over it writable once too (`Push(T: type; var v: Vec(T, 4); x: T)`), translated per distinct type and cached the way a schema production is. The wall the first record named — a schematic formal reads its discriminants from a run-time descriptor (ADR-0040) and a type cannot travel in one — was real and was gone round rather than through: a type parameter is not passed at all, it *chooses a translation*, so there is nothing for a descriptor to carry. **What stands here now is neither a clause nor a mechanism but a buffer.** An instantiation re-parses the generic's body from a saved token position, and the import loop clears the token array after each component — so a generic declared in `lib/` has a saved position pointing at tokens that are gone, and the library, which is the caller ADR-0116 wants for all of this, cannot use it. Retaining them is small and was measured rather than guessed: the string pool already survives an import, a module is about 1250 tokens, the capacity is 300000, and this compiler's own source uses 165965 of it and imports nothing — so `buffer-headroom`'s number does not move. What changes is what the buffer *means*: it would hold every source in a translation rather than one, which is a decision about a documented limit (ADR-0012) rather than a detail. Until it is taken the feature works within one source, and the four modules are still four |
