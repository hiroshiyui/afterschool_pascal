...
| | |
| --- | --- |
| **Open and ready to do** | the platforms, and only the platforms: **macOS** runs green on arm64 and its job can now fail ([below](#cross-platform-support)), with eight skips left — every one a tool the runner has not got — and a release leg still disabled; **Windows** is measured on every push rather than read ([below](#cross-platform-support), ADR-0369): two `FILE*`-over-memory functions and a `longjmp` spelling, `_Complex` and `access` not being problems under mingw-w64 at all, `pasrt_unicode.c` compiling clean — and a socket layer nobody had costed, now seven named headers of the seventeen `pasrt_posix.c` includes; **s390x** aligns `tySet` where nothing else does. **Windows is now a target the compiler emits for** even though nothing here runs one (ADR-0371) |
