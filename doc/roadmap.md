...
| | |
| --- | --- |
| **Open and ready to do** | the platforms, and only the platforms: **macOS** runs green on arm64 and its job can now fail ([below](#cross-platform-support)), with eight skips left — every one a tool the runner has not got — and a release leg still disabled; **Windows** is measured now rather than read ([below](#cross-platform-support)): two `FILE*`-over-memory functions and a `longjmp` spelling, `_Complex` and `access` not being problems under mingw-w64 at all — and a socket layer nobody had costed; **s390x** aligns `tySet` where nothing else does |
