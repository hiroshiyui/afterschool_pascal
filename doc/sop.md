...
| **Nothing finds a golden that pins an interleaving** | `lib_net_wait`'s pinned which of a buffered line and a socket's reply a round saw first, and passed on Linux for its whole life and on macOS twice before failing there (ADR-0368). Every gate here compares output byte for byte, so a schedule written into a golden is indistinguishable from a fact about the program until a platform disagrees | ADR-0205, ADR-0368 |
...
| **`runtime-coverage`'s two totals are written and never compared** | `uncovered` and `instrumented` at the head of `runtime_coverage.txt` are read only to check that *a* total is there; the per-unit rows are what gate. So they read as the file's headline claim and drift — they said 431 of 2778 while the run measured 504 of 3116, and had done since before the change that noticed. They cannot simply be gated either: they include `pasrt_posix.c` and `pasrt_task.c`, whose coverage is a property of how loaded the machine is, which is why those two are reported and not gated in the first place | ADR-0351, ADR-0354 |
