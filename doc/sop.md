...
| The dialect's **specification is enforced by nothing** | ADR-0135 wrote `doc/afterschool-pascal-spec.md`, so the dialect finally has a text an independent reader can hold — and `tests/spec/run.py`'s tag pattern still matches `@(iso7185\|extended)` only, so **no scenario can cite one of its clauses**. The document therefore states requirements that nothing checks: it is a reading, findable and attached to a clause, which is exactly what ADR-0105 claims for the suite and one step short of it. It is not the closed loop the spec was written to break — the requirements were derived from the records and verified by probe, not from `selfhost/compiler.pas` — but a spec no gate reads goes stale the way every other document here has, and the first divergence would be silent. The wiring is a third alternative in the regex plus a clause table generated from the document's own headings, and it is the next change | ADR-0135, ADR-0105, ADR-0106 |
| A **wide literal in a constant position stops the compiler** | Writing an unsigned-integer greater than `maxint` where a constant is required under `--std=afterschool` — a constant-definition, a subrange bound, an array's index-type, a set's base-type, a case-constant, an operand of a constant-expression — terminates the compiler with *case: no label matches the selector*, a case-statement in its own source having no arm for the wide literal. Reachable since ADR-0128 and found by probing a requirement while writing the specification, not by any gate: no case in `tests/dialect/` writes one in a constant position, and both conformance modes reject the literal in the lexis so nothing else could reach it. **Not fixed, because what the program should be told is a language decision** — ADR-0128 described `const c = 5000000000` as naming the digits and nothing more, which reads as an acceptance, while AP §6.4.2.6.2 requires the construct to be refused. Whichever is taken gets its own record | ADR-0128, ADR-0135 |
