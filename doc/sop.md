...
| Nothing checks an `external` declaration against the function it names | The call site is the whole of the ABI: LLVM does not check a direct call against the declaration under opaque pointers, so a wrong arity, type or function is undefined behaviour with no diagnostic. **Two properties are held.** A scalar's *width* (ADR-0364): every `int64` in an `external` catalogued as a C type that is 64 bits everywhere — a claim a person wrote, not a header read. And one function's *arity* (ADR-0371): `_setjmp`, the only foreign function the emitted module names, compared on every admitted target against what clang emits for ISO C's `setjmp` — a header read rather than a claim, and the only declaration here judged that way. Everything else about every other declaration is unchecked | ADR-0121, ADR-0129, ADR-0364 |
| **`ExtFd` is a foreign integer whose *meaning* is a platform's, and nothing asks** | The row above holds a scalar's *width*; this is the other half. `lib/dialect/pasnet.pas` binds `pasx_socket_fd` as `integer`, which `foreign-width` deliberately does not ask about — C `int` is 32 bits on every target this compiler *admits*, and that is a claim about the width and not about what the number denotes. Two callers, two different problems: `pasnet.pas:335` hands it straight back to `pasx_socket_poll`, so it need only be a token both ends agree on, while `pastls.pas:470` hands it to OpenSSL's `SSL_set_fd`, where it must be what a foreign library thinks a socket is. On a platform whose socket is not a descriptor — Winsock's `SOCKET` is a `UINT_PTR`, not an fd — the first survives and the second does not, and no gate here would say so. Nothing fails today, no such target being admitted; found by reading, which is the register's own point | ADR-0203, ADR-0264, ADR-0364 |
