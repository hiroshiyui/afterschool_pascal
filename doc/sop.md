...
| **`tools/pascalcc` has no coverage instrument of any kind, and neither has `apconfig`** | the driver is shell, so `--coverage` cannot reach it and nothing here can say a branch of it ran. ADR-0348's subcommands grew it ~200 lines; ADR-0361 took 45 of those back out by moving the project-file reader into `tools/apconfig.pas`, and what is left is an argument parser, four subcommands and a `case` over ten keys. **The reader did not gain an instrument by being written in Pascal**: `pascalc --coverage` would reach it, but `lib-coverage` measures *modules* imported by a corpus case and `apconfig` is a program that no case runs, so its ten keys and its refusal paths are asserted by `new_project.sh` and counted by nothing. `new-project` and `producttest` exercise both well by hand — every config key is asserted, two of them only after a review found them parsed and dropped — but *well by hand* is what every corpus in `doc/history.md` was opened by | ADR-0348, ADR-0350, ADR-0361 |
