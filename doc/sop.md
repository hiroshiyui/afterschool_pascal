...
- **B1. Cite the clause**, in the code comment and the commit message. A rule
  ...
  - **The number is checked to exist** by `clause-citations` (ADR-0164), which
    is the half that *can* be mechanical: over the citations in the tree — 9145 across 1505 files as this is written, and the gate prints the pair — it asks whether
...
**ADR-0236's row, added on implementation** (2026-08-29). The language server
is the first program here that lives outside `tests/`, so it is the first thing
in the tree that every corpus sweep is blind to at once. The row says so, and
says why the obvious fix is a decision rather than a chore.
...
| **`lsp/` is outside every corpus sweep** | `line-coverage`, `heap-balance`, `variant-check`, `procedure-coverage` and the `--dump-all` flag sweep are all globbed over `tests/`, and `lsp/pasls.pas` is not there — it lives in `lsp/` because a server has to be a binary an editor can be pointed at rather than one compiled into a temporary directory and thrown away (ADR-0236). So the one thing checking it is the `lsp-server` case: two recorded sessions, compared byte for byte. A leak in it is invisible, a statement never run is invisible, and a variant read through the wrong arm is invisible. The same is true of `lib/`, which ADR-0114 recorded and which the three `lib_*` cases reach only through use — but the library is at least *reached* by cases inside the glob, and this program is reached by nothing else at all. Not fixed because the fix is a decision: either the sweeps learn a second root, or the server acquires a shape that a `tests/` case can drive, and the second would undo the reason it is where it is | ADR-0236, ADR-0114 |
