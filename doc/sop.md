...
| The `structural` rows are read; the **`testable`** ones are not | `tests/spec/clauses/triage.tsv` classifies every heading, and only `testable` enters the denominator and the work queue — so a requirement filed `structural` disappears and nothing ever asks for it. That direction is now swept: every structural row of both standards and of the dialect specification was read against its own clause text on 2026-08-25, four were reclassified and nine reasons were corrected (ADR-0200). What replaces the row is the **other** direction, which nothing has ever looked at: a clause filed `testable` that in fact states no requirement sits in `pending.txt` for ever as work nobody can do, and reads as coverage owed rather than as a misfiling. There are about 350 of them, most carrying a reason that is only the clause's title — which is not an argument, and is what the structural sweep found wrong nine times on the side that *had* arguments. The sweep that would answer it is the same one, run over a much larger set and with a much weaker signal, since "states a requirement" cannot be read off the presence of `shall` | ADR-0106, ADR-0152, ADR-0200 |
