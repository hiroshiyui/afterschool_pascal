...
| **Every gate but `target32` is run at one optimisation level** | ADR-0334 found what that costs: `tests/dialect/int64_foreign.pas` declared C's `labs` as taking an `int64`, a wrong ABI on every ILP32 target, and `target32` swept it 570 sources at a time for two weeks and passed — at `-O2` the optimiser folds `labs` of a constant away before the ABI matters, and at `-O0` it answers `-3028092405585415680`. **The combination was run by no job**: `thirty-two-bit` took the default `-O2`, and the `unoptimised` job has no 32-bit libc, so `target32` skips inside it and ctest reads the skip as success. Two jobs, two axes, and the cell where they cross was empty. `thirty-two-bit` now runs the gate twice. What is **not** closed is the general question: `sanitizers`, `tls`, `fpc-differential`, `second-backend` and `llc-second-backend` each answer at one level, and the argument that the `unoptimised` job covers `-O0` holds only for a gate that does not skip in it — every gate in that list skips without a package that job does not install. Adding a second step to each is a doubling of the CI bill for gates whose defects are not optimisation-sensitive, and which of them would pay has not been measured | ADR-0325, ADR-0328, ADR-0334 |
