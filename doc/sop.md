...
| **The emitted `wasm32` datalayout assumes an LLVM that names `i128` for the target** | The line the module states is clang 19's, and an older one — Ubuntu 24.04's, and the arm64 runner's — states no `i128:128` for `wasm32`, so it aligns an i256 to 8 where this compiler says 16. clang *overrides* the module's line with its own for the `--target=` it is given (ADR-0156), so on such a toolchain a set inside a record would be laid out two ways — the ADR-0028 defect. `target-layout` reports it and abstains, saying which layout the target is supported against, and that is all it can do: **no wasm program runs here**, so there is no behavioural oracle behind the abstention | ADR-0028, ADR-0156, ADR-0383 |
