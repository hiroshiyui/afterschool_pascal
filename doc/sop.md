...
| **A generic is translated in its client, and that costs two things** | Instantiated by two translations it is emitted into both, each under its own counter — duplication and not a wrong answer. And its body may call only what its clients can reach: a module's internal routine called from one is an undefined symbol in the client, naming a counter, and `PasContainer` exports two helpers no caller wants as the workaround. Both fixes are one linkage decision — a name shared across translations — which ADR-0411 has since made for a *method* and not for these | ADR-0211, ADR-0212 |
