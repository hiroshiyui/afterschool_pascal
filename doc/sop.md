...
| **Two gates flaked once each on 2026-09-06 and neither is explained** | `fpc-differential` failed in a full run and passed on rerun and on a second full run; and a coverage sweep saw `tests/extended/lib_strings.pas` fail to compile with *unexpected character '}'* at 10:53 — the closing brace of its leading comment, as though the opening `{` had not been seen — which did not reproduce in four further full sweeps, nor by hand. The second is the one that matters: it is the shape of a **short read** in the source reader, and if it is that then every compilation in this tree is subject to it at some rate nobody has measured. Both were seen while several harnesses ran at once, which is the ordinary condition here (ADR-0281) and not an excuse. Nothing catches an intermittent: every gate here is a single run, and a defect with a rate rather than a trigger passes almost all of them | ADR-0281, ADR-0351 |
