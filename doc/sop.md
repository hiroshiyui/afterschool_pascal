...
| One conformance defect is **known and unfixed**: a subrange whose bounds are not constants is refused anywhere but an array's *index-type* | §6.4.2.4 writes `subrange-bound = expression`, so `var x: 1..m`, `type t = 1..m` and `array [1..m] of 1..m` are legal and are refused. The reason is that a bound only *works* in an index-type: the subscript check reads it out of the descriptor §6.2.3.8 b) filled, where every other subrange's bounds are read by the range check at a store, which compares against the two numbers on the type. Accepting it was worse than refusing it and that is what ADR-0127 changed — `a[1] := 2` over `array [1..m] of 1..m` with `m = 3` trapped with *value out of range (1..)*, a legal store stopped by a comparison against an upper bound nothing had read. Fixable and not a decision: the store's check has to read the descriptor as the subscript check does, and the message has to be built by the runtime as an array's already is (ADR-0040). The other two halves of the entry this replaces are fixed — the variable by ADR-0113 and the type-definition by ADR-0127 | ADR-0107, ADR-0113, ADR-0127 |
