...
| A **generic routine cannot cross `--import`**, so the library cannot use it | AP 6.7.3.10 re-parses a generic's body from a saved token position, and the import loop clears the token array after each component — so a generic declared in `lib/` points at tokens that are gone. Nothing detects it as such: the saved position is an index into an array that still exists and now holds a different source, so what a client would get is not a diagnostic but *whatever those tokens now say*, parsed in the generic's scope. No corpus case can reach it, because no module declares a generic — and none can until this is closed, which is the circularity that makes the row necessary. Closing it is retaining the tokens (measured: a module is ~1250 of 300000, and the compiler imports nothing so `buffer-headroom` does not move); what it costs is that a fixed buffer starts meaning *every source in a translation* rather than one | ADR-0211, ADR-0012 |
