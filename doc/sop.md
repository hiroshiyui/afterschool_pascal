...
| A new type **kind** is checked exhaustively in case-statements and nowhere else | `kind-exhaustive` reads every `case … of` over the twelve enumerations and requires every constant to be named, which is what makes adding one to `typeKind` safe *there* (ADR-0124, ADR-0145). It cannot see a **predicate**. `IsMemory` is `IsStructured or IsOwned or IsVarString`, a boolean expression with no arms, and adding `tyText` left it answering false — so the relational operators took a text for a simple type and emitted `icmp` on an aggregate, which is ADR-0139's defect reproduced by adding a kind (ADR-0191). **Three of these in three increments**: that one, the codegen comparison dispatch, and `EmitAssign` selecting the string store with `IsStringType` — which sent a text target to the schema tuple-comparison, where a *string's* capacity was compared against a text's and stopped the program (ADR-0193). Each is a predicate used as a guard for "does this take the string path?", and none is a case-statement. It was found by probing every operation against the new type by hand, on the first day it existed; nothing would have found it otherwise, the corpus having no program that compares one. What would close it is a sweep asking each type-classifying predicate about every kind and requiring an argument for each answer — `predicate-callers`'s shape, aimed at the predicates rather than at one predicate's callers — and it does not exist | ADR-0139, ADR-0146, ADR-0191 |
