...
- **B1. Cite the clause**, in the code comment and the commit message. A rule
  ...
  - **The number is checked to exist** by `clause-citations` (ADR-0164), which
    is the half that *can* be mechanical: over the citations in the tree — 10 384 across 1668 files as this is written, and the gate prints the pair — it asks whether
...
| A **missing join** is not caught by any case | AP 6.9.3.12.1 joins every task a block spawned before releasing anything of that block's, and the emitter puts the join first in the epilogue. Removing it entirely leaves `tests/dialect/concurrency.pas` **green**: every task in it finishes before its block ends, so nothing observes the difference. AP 6.7.8.2 removed the sharpest consequence -- a task can no longer read an enclosing activation's variables, because it can no longer name them -- but the property is asserted by argument and not by a case. What would close it is a task still running when its block ends *and* whose continued running is observable, which is a race to write deliberately and so a test that is hard to make deterministic | ADR-0201, ADR-0268 |
...
| Nothing **generates** an input the compiler has not been shown | ADR-0261 put the corpus under AddressSanitizer, UndefinedBehaviorSanitizer and LeakSanitizer, which is the cheap half of this: it watches the runtime's own storage, which nothing did before, and it found a leak on its first run that `heap-balance` records as balanced -- `pas_bind` copies an external name and `pas_file_done` never frees it. What it runs is still **what somebody already wrote**. A hand-written lexer and parser over fixed buffers (ADR-0012) is the canonical fuzzing target, and `selfhost/torture.pas` and `selfhost/badparse/` are hand-written corpora -- so ADR-0067's sentence, *a claim no test names is a claim nothing checks*, applies to crash-resistance exactly as it applied to conformance. A fixed buffer that overflows on an input nobody thought of is the failure this cannot see, and it has failed as a **build** twice already (ADR-0126, ADR-0148). What would close it is a generator over the token grammar and a corpus of its own, which is a job rather than a gate | ADR-0012, ADR-0067, ADR-0126, ADR-0183, ADR-0261 |
...
| A generic whose parameter-form names a **schema the caller cannot see** infers nothing | AP 6.7.3.10.4 reads a type parameter out of a production by looking the schema's name up *at the call* (ADR-0254), so a generic exported by a module whose schema is private to it, or simply not imported here, determines nothing through that formal and the types must be written out. It is a graceful degradation and not a wrong answer, and the lookup is deliberate rather than a comparison of spellings: a same-named schema of the caller's own would otherwise bind a type the callee never meant. No case in this tree has met it, `generic_import.pas` importing the schema along with the generic; what would fix it is carrying the callee's region into the unifier, which is a decision about where a name is resolved and not an oversight | ADR-0254, ADR-0212 |
...
| **Nothing fails when this tree's own source acquires an unprotected read-only `var` parameter** | ADR-0283's warning found 54 of them and the tree now reports zero, but that zero is held by nothing: a `.warn` sidecar makes a *test case* fail, and `selfhost/` and `lib/` have no sidecars -- the compiler's own build simply prints the warning and succeeds. So the count can drift back up one increment at a time and only a reader of the build log would know. It is also a **fixed point** rather than a count, so a gate over it would have to iterate to convergence rather than compare a number, which is why one was not written here | ADR-0283 |
