...
| The **frame-offset comparison** across targets was made once, by hand | ADR-0155 measured that LLVM lays out every one of this compiler's frames identically under the x86-64 and aarch64 datalayouts -- 4501 sizes and offsets, including the `i256` in a record that is ADR-0028's segfault -- and that is what says `LlSize`/`LlAlign` need no change for an LP64 little-endian target. Nothing re-runs it. `target-sizes` checks the two *opaque struct* sizes on every target a compiler is installed for and says nothing about layout; a datalayout that diverged between two targets would go unnoticed until someone tried to port. Closing it means generating the probe from a build's own IR rather than from a scratch file, which belongs with the port itself | ADR-0028, ADR-0155 |
