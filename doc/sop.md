...
| A **release the runtime cannot be asked to make** is watched by nothing | AP 6.4.14.3 requires an owned pointer's storage to be given back when the activation terminates by a `goto` or a `halt`, and the block epilogue is the only exit that does it. The files and handles *inside* the owned variable are released on those paths, because each is registered with the runtime individually — so the observable resource comes back and the `malloc`ed block does not, which is precisely the difference no oracle here can see. There is no memory-accounting oracle at all: `ctest`, the goldens, `difftest`, `verify/` and the BSI suite all read what a program *prints*, and a leak prints nothing. The `-fsanitize` runs of `security-audit` are the nearest thing and are not a gate. What found the original defect this feature closes was `ulimit -n 64` and a counting loop — an experiment, run once, by hand | ADR-0181 |
