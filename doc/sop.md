...
| The coverage gates are **ratchets**, and two of the five hold a number no ratchet can | a ratchet fails when the count rises, and a line that stops being reached for a bad reason is invisible to it; the per-procedure or per-module breakdown is what makes a regression nameable. It was one file and is now `line_coverage.txt`, `branch_coverage.txt` (ADR-0104, ADR-0274), `lib_coverage.txt` (ADR-0350), `runtime_coverage.txt` (ADR-0351) and `lsp_coverage.txt` (ADR-0352). **`lib` and `lsp` fail in both directions since ADR-0353's commit**, being pure computation over fixed inputs and identical on every run: an improvement left unrecorded keeps the old floor and the slack accumulates until the gate admits whatever nobody chose, which is `verify/`'s KNOWN_GAP rule said about a number. `line`/`branch` were chosen one-way (ADR-0104) and the choice stands. **`runtime` is the interesting one** (ADR-0354): made two-way, it failed on CI at once — 433 lines never run where the file said 436 — and the delta reproduced here only on **one core under a busy loop**, exactly three lines of `pasrt_task.c`: the arm of `select` that runs when its deadline beats the receive. That coverage is a property of how loaded the machine is, so it cannot be held in *either* direction — a faster box than the one that wrote the file would report a loss — and the two units holding a wait with a deadline are printed and not compared, while the two holding none are gated both ways. What is left uncounted by any gate is therefore: two ratchets by choice, and a few lines of two units by nature | ADR-0104, ADR-0274, ADR-0350, ADR-0351, ADR-0352, ADR-0353, ADR-0354 |
