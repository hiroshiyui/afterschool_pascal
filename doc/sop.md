...
| `coverage.py` sees the sources, not the harnesses that build their own compiler | What `irtest.py`, `producttest.sh` and `verify.py` drive is invisible; the *flags* half is closed by sweeping `--dump-all` and `--coverage` over every source | ADR-0103 |
