...
| **Nothing here tests an i386 without SSE2, and two things break on one** | `target32` names `-march=pentium4` since ADR-0345, because clang 19 compiles this triple for `i686` and clang 21 for `pentium4` and the gate cannot mean two things. What the pin buys is that the gate measures pointer size, which is what it is for; what it costs is that x87-only i386 is now **untested rather than working**. Two cases fail there and were measured failing: §6.7.6.3 defines `round` by equivalence to `trunc(x ± 0.5)` and an 80-bit register makes the two disagree at `-0.49999999999999994`, and — the one that matters — D.32's `sqr` error goes **undetected**, `sqr(-1e200)` being an ordinary finite number in a register with a fifteen-bit exponent. Each shows at one optimisation level and not the other, whether the intermediate reaches memory being the optimiser's decision. Whether this compiler supports such a processor at all has never been decided; `-msse2 -mfpmath=sse` is what would close both if it does | ADR-0325, ADR-0334, ADR-0345 |
| **Nothing detects a harness that ignores an environment variable it is handed** | ADR-0335 is the instance: `sanitize.sh` compiled 383 corpus programs at a level it chose itself and read `AFTERSCHOOL_PASCAL_OPT` not at all, so `AFTERSCHOOL_PASCAL_OPT=-O0 ctest` reported the whole suite green at `-O0` with two of its cases at `-O1` — and CI's `unoptimised` job made that claim on every push. `require-consistency` (ADR-0330) does exactly this comparison for `*_REQUIRE`, in both directions, and the same shape for `AFTERSCHOOL_PASCAL_*` would have to know which harness *should* read which variable, which is a judgement rather than a comparison: `foreign-reserved` has no business honouring an optimisation level and `target_layout.py` none honouring a runtime path. What is available cheaply is the negative half — a variable a harness sets in a subprocess environment and no harness reads — and it is not built. **It has now happened twice**: `llc_check.sh` ignored `AFTERSCHOOL_PASCAL_TARGET`, so the `second-backend on aarch64` job compared x86-64 modules on an ARM machine and had never passed since the day it was added (ADR-0345). Both instances are a harness that answered, in good faith, about something other than what it was asked | ADR-0330, ADR-0334, ADR-0335, ADR-0345 |
