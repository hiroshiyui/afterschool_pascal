...
| **What the shell *emits* for a role is asserted by nothing, and the table it emits from now is** | ADR-0389 split a cell's **role** from its colour so a golden could hold the first without the second, and ADR-0391 had `PutRow` split a row into runs and write one `SetColour` per run. Both records promised a row here and neither wrote one, which is the register's own decay shape a third time. ADR-0393 closes the half that could be closed: `tui-palette` reads `RoleColour` and holds every pairing to an extreme against a colour and to a floor, and holds every declared role to being drawn by some session — that second claim found `crPrompt` declared, coloured, lettered and drawn on no screen for the whole life of ADR-0392, with all fifteen sessions green. What is still uncovered is the **emission**: that the runs written match the role plane, that the sequences are the ones `PasTerm` means, and that a terminal does with the eight ANSI colours what xterm does. The first two were checked once by hand under a pseudo-terminal; the third is not checkable here and is why the rule is structural and the ratio only evidence | ADR-0389, ADR-0391, ADR-0393 |
