...
| The **string pool's** headroom is not measured | `buffer-headroom` counts the token array exactly -- `--dump-tokens` writes one line per token and §6.1.7 forbids a newline in a string-literal, so the line count *is* the count -- and can say nothing about `pool`, because `PoolAdd` is called from Sema and from CodeGen as well as from the lexer: a type's alias name and a trap message are both interned. So a count over the token stream is a lower bound (442,625 of 1,000,000 for this source) and not the size. It matters because the pool is the array that hit its ceiling first, four days before the tokens did, and both times the failure was a *build* that could only be cleared by an out-of-cycle reseed. A `--dump-limits` writing `poolLen` and `tokCount` after a compilation closes it exactly, for every input rather than for this source, and is the move if the pool bites again | ADR-0095, ADR-0126 |
| A gate that holds **both halves** of its comparison cannot fail | `foreign_reserved.py` kept `COMPOSED`, a regex copy of the rules `ReservedForeignName` applies without a list. Adding `frame[0-9]+` to it without adding it to the compiler left the gate green over a name the compiler still accepted (ADR-0144). Closed for this gate — it now compiles a probe, harvests the `@names` the IR actually contains, and offers each back to the compiler — but the shape is general and other checks here read the compiler's *source* to decide what it does: `kind_exhaustive.py`, `buffer_headroom.py` and `reserved_words.py` all parse `compiler.pas`. Each is a weaker oracle than asking the built compiler, and only the last of the three does both | ADR-0121, ADR-0144 |
| Two `external` declarations naming one linker symbol emit two `declare`s | LLVM rejects a redeclared global, so `function a1(x: integer): integer; external 'abs'; function a2(x: integer): integer; external 'abs';` is refused with *invalid redefinition of function 'abs'* -- an error about a file nobody wrote, which is the exact shape ADR-0121's `foreign-reserved` gate exists to prevent from the *other* direction. Present since ADR-0121 and found while probing ADR-0128's boundary; nothing in the corpus binds one symbol twice. The fix is to key the declaration list on the linker name rather than on the Pascal symbol, and it is not done here because it belongs to the FFI rather than to the type | ADR-0121, ADR-0128 |
...
| A dialect scenario cannot reach **6.13.1** | `doc/afterschool-pascal-spec.md` is now cited by 95 citations across a 97-scenario suite, and 45 of its 46 testable clauses have one (ADR-0135's wiring). The one that does not is 6.13.1 -- every program-component of one program shall have been translated under the same `--std` -- because `tests/spec/run.py` compiles a single program and a scenario has no way to ask for a second component, let alone one built under another mode. It is covered by `tests/dialect/` and by the link-time diagnostic `tools/pascalcc` translates, and it stays in `pending.txt` rather than being triaged away, because triaging a clause the harness cannot reach as `structural` would be a lie about the clause rather than about the harness | ADR-0119, ADR-0135 |
...
| Nothing sweeps the predicates a new type-rule newly satisfies | AP §6.4.5 makes two slices compatible, for parameter passing; the relational operators ask compatibility too, and `a[1..2] = a[3..4]` reached CodeGen and emitted invalid IR (ADR-0139). That is ADR-0058's sentence a **second** time -- a permission granted in a shared predicate leaks to every caller -- and the sentence was already written in `CLAUDE.md` when the second one landed, so knowing it is demonstrably not enough. Eighteen other positions were probed after the fact and every one refuses a slice, but that is a probe over the positions someone thought of. What would close it is mechanical: for each predicate a new type is taught to satisfy, enumerate its callers and require an answer per caller. Nothing does that, and the next extension of `Assignable` has nothing looking over its shoulder | ADR-0058, ADR-0125, ADR-0139 |
