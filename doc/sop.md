...
| **Nothing detects a harness that ignores a path, target or flag it is handed** | Three in a week: `sanitize.sh` and `AFTERSCHOOL_PASCAL_OPT`, `llc_check.py` and the target, `seed_current.py` and an absolute path. `require-consistency` does this for `*_REQUIRE`; the same for `AFTERSCHOOL_PASCAL_*` needs a judgement about which harness should read which | ADR-0330, ADR-0335, ADR-0345, ADR-0347 |
