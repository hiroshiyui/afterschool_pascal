...
| **Every target the tree compiles the runtime for is `*-linux-gnu`** | `target-sizes` builds `runtime/*.c` for nine triples and all nine are Linux, so nothing here notices the runtime drifting further from a platform that is not POSIX. Measured once by hand against mingw-w64 (doc/roadmap.md's Windows row); no gate repeats it, and the compile stops at the first missing header, so even that measurement is a floor | ADR-0155, ADR-0161 |
