...
| **`CONF068` is the one CONFORMANCE program the BSI suite has that this compiler refuses** | §6.4.3.4, a file as a field of a variant part, refused deliberately (ADR-0070). All 812 programs were re-run against the catalogue during ADR-0162's audit and all 812 agree, so this is the only place in the one corpus nobody here wrote where over-strictness could still be hiding. It was outside every reader's brief and is the obvious target for the next audit | ADR-0070, ADR-0162 |
