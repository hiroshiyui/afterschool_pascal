...
| **`runtime-nonposix` holds one bit per unit, and a second blocker hides behind the first** | A unit blocked for a new reason while still blocked for an old one reads `blocked` either way; the *names* inside a header that is present are not catalogued the way `runtime-isoc` catalogues them. **What would close it is `runtime-isoc`'s own mechanism** — shim every catalogued gap and require the unit to compile, which is how that gate proves its catalogue complete — and today that means stubbing seven headers' worth of sockets and terminals for a platform that is deferred (ADR-0374). It bites exactly one unit: `pasrt.c` and `pasrt_unicode.c` compile, `pasrt_task.c` is `crt`, and `pasrt_posix.c`'s blockers are *headers*, which are catalogued exactly. So the hidden layer is unknowable until the headers exist, which is a fair thing for this register to say | ADR-0161, ADR-0369, ADR-0374 |
