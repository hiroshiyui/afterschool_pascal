...
2026-09-11 (before v3.11.0: **four stale rows in 102, and one of them was the
register's own most-warned-about shape doing real damage.** *Nothing checks
that a harness works only inside the directory it made* ended with the words
*every local parallel run now exercises it* — a row naming its own closing
condition, and wrong, because what wrote outside its directory was not a
harness. Every harness did work in a directory it had made; the **program
under test** was started with no `cwd` at all and ran in the invoker's, so the
one corpus case that names a file relatively raced its own concurrent copy and
failed on CI on three jobs across two pushes while a local parallel run passed
(ADR-0406). The row could not have been read as covering that, and the
sentence is why nobody tried. The other three were counts a cut moved:
`runtime`'s report-only units are three since ADR-0405 and the register said
two, `runtime-nonposix` bites one of *five* units and the register said four,
and the directory walk whose error paths nothing can arrange is in a different
file now. Every one of the four was found by reading the row and none by a
gate — `quoted-numbers` holds what a *document* quotes and a count written into
a sentence here is not catalogued unless somebody catalogues it.)

2026-09-12 (after ADR-0411, and the first audit asked for by name rather than
taken on a clock: **two rows of 108 had closed and neither had been struck, and
three other documents carried one of them as live.** §6.4.3.3's region *is*
asked of a constant occurrence -- `array [1..fred]`, `array [1..fred+1]`,
`set of 1..fred` and `string(fred)` beside a field `fred` are all refused, with
a message that names the region -- and ADR-0134 did it, saying so in the
comment on `ErrorFieldNotA` and in `doc/implementation-defined.md` §6.1, which
records the entry as the *last* one and closed. `CLAUDE.md` and one of
`doc/design-digest.md`'s two bullets on the clause still said it was not asked,
and the digest's own other bullet said it was fixed -- a fact stated twice,
disagreeing with itself, which is the shape ADR-0388 removed from a program and
nothing removes from prose. And an `unreachable_diagnostics.txt` entry naming
no message **is** caught: `listed - uncovered` holds it whether the message was
deleted or a golden now names it, and a probe entry is reported. What made that
row read as open was the gate's own wording, which says *a golden now names
it* about a message nobody can write; it now says which of the two happened.
The other two findings were a headline and a pair of counts: the `tls` row led
with a defect fixed six days earlier rather than with the gap it was recorded
for, and *204 of the 390 testable rows* was neither reproducible nor current --
re-measured, 145 of 399 carry no reason at all. Every one of the four was found
by reading a row and probing what it claimed, and none by a gate.)
...
| **Raw mode is asserted in part, and three of its six claims still by nobody** | `ctest` has no terminal: `tests/dialect/lib_term.pas` runs with every descriptor a regular file, so it pins the *negative* path — `IsTerminal` false, `TermSize` answering `errAbsent` rather than a plausible number, `EnterRaw` refusing and changing nothing. **ADR-0402's `tui-terminal` reaches the positive path for three of the six**, because the editor it drives cannot work without them: a read answers on **one keystroke**, every script being a sequence of single bytes each provoking a frame, so canonical mode is cleared; `TermSize` answers the size the harness set with `TIOCSWINSZ`, so it reads a real window rather than a plausible number; and the program **exits** after two Ctrl-Q with the terminal usable, which exercises the restore. What no case reaches: that the flags cleared are exactly the right ones — an echo left on writes bytes *between* frames and the harness parses frames — that the settings put back are the settings taken rather than a plausible set, a real window size that *changes*, and `EnterRaw`'s `errFull` arm, which needs a first `EnterRaw` to succeed and so is reached in **neither** direction. **ADR-0262 promised this row and it was never written**, which is the register's own decay shape found from the other side; ADR-0381 made it matter more, an editor being a program that lives in raw mode | ADR-0262, ADR-0381, ADR-0402 |
| **East_Asian_Width has no conformance file, and it is the one Unicode property here nobody else checks** | `unicode-conformance` is this tree's only oracle written by neither this processor's author nor its specification's, and it earns that by running Unicode's own `NormalizationTest.txt` and `GraphemeBreakTest.txt`. AP 6.4.15.13's table has no such file: Unicode publishes East_Asian_Width and publishes no test for it. Regenerating the header and diffing it is a check that the *generator* is stable, not that the table is right. **Mostly closed by ADR-0400**, which compares against ICU's reading of the same two files: 1 112 064 code points, exact agreement, no catalogue. What is left is that both could transcribe the same upstream change — two readings agreeing is not the same as either being right — and that on a machine whose ICU predates the pinned Unicode the gate **abstains** and this row is open again there | ADR-0189, ADR-0395, ADR-0400 |
| **A terminal doing with these bytes what the sequences say is checked by nothing, and so is a terminal's `COLORTERM` telling the truth** | ADR-0389 split a cell's **role** from its colour so a golden could hold the first without the second, and ADR-0391 had `PutRow` split a row into runs and write one `SetColour` per run. Both records promised a row here and neither wrote one. ADR-0393 closed the *table*: `tui-palette` reads `RoleColour` and `RoleRgb` and holds every pairing to an extreme against a colour and to a floor, and holds every declared role to being drawn by some session — that second claim found `crPrompt` declared, coloured, lettered and drawn on no screen for the whole life of ADR-0392, with all fifteen sessions green. **ADR-0402 closes the emission**: `tui-terminal` drives the real editor under a pseudo-terminal and requires every run it writes to be one the model decided, at the same columns, with the same cells and in the colour that role's table gives, over *both* tables — so a shell painting every row in one colour, or emitting from the table the terminal did not ask for, now fails. It is not the binding ADR-0262 declined: the pseudo-terminal is Python's, as `lsp/run.py`'s pipe is. What is left is the half that is not checkable here at all — that a real terminal renders these sequences as they are meant, and that an emulator setting `COLORTERM` understands the twenty-four-bit form, there being no query a terminal lacking it answers safely. A terminal that lies gets a scheme it cannot render, and the cost is bounded by the fallback being held to the same floor | ADR-0389, ADR-0391, ADR-0393, ADR-0394, ADR-0402 |
...
| **`runtime-nonposix` holds one bit per unit, and a second blocker hides behind the first** | A unit blocked for a new reason while still blocked for an old one reads `blocked` either way; the *names* inside a header that is present are not catalogued the way `runtime-isoc` catalogues them. **What would close it is `runtime-isoc`'s own mechanism** — shim every catalogued gap and require the unit to compile, which is how that gate proves its catalogue complete — and today that means stubbing five headers' worth of sockets, terminals and processes for a target no program here is built for yet (ADR-0382). It bites one of the five units (ADR-0405 cut the POSIX one in two): `pasrt.c`, `pasrt_unicode.c` and `pasrt_file.c` compile, `pasrt_posix.c`'s blockers are *headers* and are catalogued exactly, and `pasrt_task.c` is blocked by something a header refuses to declare — the thread routines — with nothing to say whether a second waits behind it. The toolchain's three flags are this row met early and dodged three times: without them a build flag, the *host's* headers, and a permissive `<pthread.h>` would each have stood in for the real answer. **And the fourth flag was this row biting** — the gate compiled `-std=c11` where the build uses `-std=gnu11`, so `pasrt.c` was catalogued blocked on a name the target has (ADR-0383) | ADR-0161, ADR-0369, ADR-0380, ADR-0382, ADR-0383 |
...
| Nothing checks an `external` declaration against the function it names | The call site is the whole of the ABI: LLVM does not check a direct call against the declaration under opaque pointers, so a wrong arity, type or function is undefined behaviour with no diagnostic. **Three properties are held.** A scalar's *width* (ADR-0364): every `int64` in an `external` catalogued as a C type that is 64 bits everywhere — a claim a person wrote, not a header read. And one function's *arity* (ADR-0371): `_setjmp`, the only foreign function the emitted module names, compared on every admitted target against what clang emits for ISO C's `setjmp` — a header read rather than a claim, and the only declaration here judged that way. And one symbol's *signature* being one signature (ADR-0376): a C function declared in more than one module under `lib/` or `lsp/` must have one representation or a catalogued argument, compared by representation rather than spelling because two modules legitimately name one C type differently. Everything else about every other declaration is unchecked | ADR-0121, ADR-0129, ADR-0364 |
...
| Nothing runs a program with **no writable temporary directory** | An auxiliary file is an exclusive `fopen` in `/tmp` and then in `.` (ADR-0390); the five lines that answer when neither will have it — the collision retry, the refused `remove`, and the two giving-ups — are the five `runtime-coverage` reports as newly uncovered, and no harness here can make `/tmp` refuse | ADR-0390 |
...
| **A catalogued partial case is only as good as its argument, and nothing re-reads the argument when the enumeration grows** | `kind-exhaustive` compares a site's `N of M` against `partial_cases.txt` in both directions, which catches a numerator that moves and a denominator that moves. What it cannot read is the **reason** beside the row. `DumpTypeExpr`'s row is argued as *each names its own third* — expressions, statements, type-denoters — and adding `nkDyn`, a type-denoter, falsified that sentence while 15 still equalled 15, so `--dump-ast` over any program containing a `dyn` stopped the compiler with `case: no label matches the selector` (ADR-0408). **`procedure-coverage` caught it** and `kind-exhaustive` did not, by ADR-0269's rule that the compiler must *survive* every invocation — which is a second gate holding the first one's blind spot and is why that rule was written. What would close it is a row saying which constants a subset is *closed over* rather than how many, and that is a catalogue of a different shape | ADR-0145, ADR-0269, ADR-0408 |
| **A trait-keyed call records no use, so no editor can find a trait routine's definition** | Sema records a use where it *resolves* one (ADR-0111's argument, ADR-0230's), and `DispatchTrait` is the one resolution that does not: neither the function-designator arm nor the procedure-statement arm calls `NoteUse`, so `--dump-uses` reports nothing for a call to a trait routine and `definition`, `references` and `rename` all miss it in an editor. Found while adding the procedure half (ADR-0407) by asking the dump rather than by a gate — **no oracle here can see it**, because `--dump-uses` has no catalogue of what it *should* contain and a use that is absent looks exactly like a name that was never written. It is pre-existing and on both sides, which is why the procedure arm was written to match the function arm rather than fixed on one side alone: one clause resolving a name two ways is the defect this row is about, not the fix | ADR-0111, ADR-0340, ADR-0407 |
| **Three predicates decide what a function-designator is, and nothing compares them** | `IsCallValue`, `IsDesignator` and `CalledSym` answer *is this a call*, *is this a variable* and *which routine* about the same node, and every rule that needs one of those asks it by name (ADR-0179). They must agree — a spelling that is a call to the first and a designator to the second reaches a `var` parameter, which is how AP 6.7.10.4's method-designator passed a value where the callee wanted an address until `IsDesignator` was told (ADR-0410). **`kind-exhaustive` cannot see this**: the disagreement was inside an arm it already counted, not a missing arm, so its `N of 69` never moved. What would close it is a gate asking all three about one node and refusing an inconsistent triple, and it is declined here because the answer is only defined for the handful of node kinds that can be either — the catalogue would be a list of the cases somebody thought of, which is what these gates exist not to be | ADR-0179, ADR-0410 |
| **The one indirect call the emitter makes has no lowering proof, and `verify/` is the wrong instrument for it** | AP 6.7.11's trait object loads a code address and a static link out of a table and calls them (ADR-0409). `verify/` proves a lowering against a property-style statement of the standard, and the property here — *the call reaches the routine the table names* — is the emitted code written a second time, which is the nil-check's shape that ADR-0013 says dilutes what *no known gaps* means. So no rule was written, deliberately, and what holds the lowering instead is a behavioural case under `valgrind-corpus` and `sanitizers`, plus the heap balance. **What none of those sees is a table built wrongly in a way every entry agrees about** — a slot order, say, that is self-consistent across all implementations of one trait; `tests/dialect/dyn.pas` declares two of its four implementations out of the trait's order so that the commonest such mistake is caught, and that is a case rather than a proof | ADR-0013, ADR-0409 |
| **Nothing checks that a harness works only inside the directory it made** | Every writing harness calls `mktemp -d` or `mkdtemp` and asks for a port; a grep proves the directory exists and not that nothing is written outside it — `format_check.py` shipped writing a fixed path with every gate green. Declined rather than written as a proof that is not one. **Its closing sentence used to read *every local parallel run now exercises it* and that was the second half of the defect** (ADR-0406): the harnesses did work in a directory each had made, and the *program under test* did not — it was started with no `cwd` and ran in the invoker's, so the one case naming a file relatively raced its own concurrent copy. A local parallel run exercised it perfectly and lost the race about twice a day, on CI, on three jobs. The row is the same and what it does **not** cover is now one thing smaller: a case runs where nothing else does, and a *harness* writing outside its own directory is still proved by nobody | ADR-0281, ADR-0406 |
...
| **A number quoted from a gate that runs only in a job of its own is checked nowhere** | `quoted-numbers` reads what the gates *last printed*, from the ctest log, and runs after the suite in the `test` job. A gate that needs a toolchain that job has not got skips there and answers in a job of its own — `wasm32` and `runtime-nonposix` are the two — so its rows are reported as *unchecked* every time and nobody reads that line. Found by hand on 2026-09-12, while auditing this register: three documents said **563 of 598** corpus programs run as WebAssembly where the gate had been answering **570 of 605**, and the rows watching exactly those sentences were in the catalogue and had never once been evaluated. The fix is to run the check in those jobs too, against their own output, and it is not built — what stops it being a line is that the check's evidence is a *ctest* log and those gates are invoked directly | ADR-0379, ADR-0385 |
| **`wasm32` support requires an LLVM that names `i128` for the target** | The line the module states names `i128:128`, and an older LLVM — Debian trixie's clang 19, Ubuntu 24.04's, the arm64 runner's — does not, so it aligns an i256 to 8 where this compiler says 16. clang *overrides* the module's line with its own for the `--target=` it is given (ADR-0156), so on such a toolchain a set inside a record is laid out two ways — the ADR-0028 defect. **This is no longer unwitnessed**: `tests/sets_records.pas`, the case that decision exists for, was the single failure in 598 when the job ran on trixie. Both `target-layout` and `wasm32` now abstain where this clang and this module disagree about a modelled field, printing both lines, and the CI image is `debian:testing` so the measurement can be taken at all. What stays unchecked is the requirement itself: nothing here fails when a *reader* is on an older LLVM — they get a skip and a printed reason | ADR-0028, ADR-0156, ADR-0383, ADR-0385 |
