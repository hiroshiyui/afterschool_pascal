...
**Every audit is written up in
[history](history.md#the-audits-as-they-were-written)**; what stays here is the
date and the finding, because the pattern across them is the thing a reader of
this register needs and the working is not.

| Read end to end | What it found |
| --- | --- |
| 2026-08-25 | the first read-through, four stale of 57 (ADR-0197) |
| 2026-08-28 | six stale of 67, and a row that was missing |
| 2026-08-29 | five kinds of decay, two of them outside this file |
| 2026-09-01 | the quoted numbers, and three records no document knew of |
| 2026-09-06 | a row whose own closing condition a release had met |
| 2026-09-07 | a premise a newly admitted target falsified |
| 2026-09-08 | `langspec-audit` over the concurrency clauses: seven defects, four readings left unsettled (ADR-0365) |
| 2026-09-08 | the first macOS run: nine failures, **five of them this tree assuming Linux** where nothing had ever asked |
| 2026-09-09 | the register current and six counts around it stale — the **second** time the decay was all outside |
| 2026-09-10 | the register current again, and five documents stale — the **third** |
| 2026-09-11 | four stale of 102, one of them a closing condition that was *wrong*: what wrote outside its directory was the program under test and not a harness (ADR-0406) |
| 2026-09-12 | two rows of 108 closed and never struck, one of them carried as live by three other documents; and two extensions of the language with no clause |
| 2026-09-13 | the register **current** at 102 rows and two of them understated: a generic's third cost, found the same day and paid at the gate (ADR-0416), and the thin-corpus row, which the library rewrite gave two more instances (ADR-0413, ADR-0415). The arena producer count re-counted and still **eight** |

Read the pattern rather than the rows: **three of the twelve found this
register clean and everything around it stale**, so an audit that stops at this
file stops early — and **every finding in the last four was found by reading a
row and probing what it claimed, none by a gate.** `quoted-numbers` holds what
a *document* quotes; a count written into a sentence here is watched by nobody
unless somebody catalogues it, and §7 has no catalogue row.
...
re-counted 2026-09-13 — and the `-O1`/`-O3` row, still a judgement.
...
| **Nothing checks that a rule stated elsewhere has a clause** | AP 5.7 makes the specification the only place a rule of this language is stated and every other document a summary of it. `spec-clause-traceability` holds the other direction — every testable clause *there* is cited by a scenario — and no gate asks whether a sentence in `README.md`, `doc/tour.md` or `doc/implementation-defined.md` §5 corresponds to one. Two did not, for as long as each had existed: `halt`'s exit status and the underscore in an identifier were extensions of the language with no clause, so a reader holding the specification did not know either was legal. Both were found by reading the register's extension list against the document's headings, on 2026-09-12, and by nothing else | ADR-0135, ADR-0379 |
...
| `-O0` and `-O2` are each run, never **compared** | A case where the two differ and both look plausible passes twice; only `--crosscheck` compares them, over its own program. `irtest.py` links at `-O0` by default, which is how ADR-0220 was found, and must not be leaned on -- it honours `.opt` and `AFTERSCHOOL_PASCAL_OPT` since ADR-0366, so that default is what a run did not override rather than the only level it can reach | §6, ADR-0220 |
| `-O1` and `-O3` are unexercised | Judged not worth a third and fourth sweep | — |
| **No ctest case can tell a composed linkage name from a counter** | Every harness here hands each translation the same components in the same order -- `run_test.py` compiles a `.components` list cumulatively and then hands the client all of it -- and two translations that walk the same components in the same order allocate the same counters. So a routine named by a counter linked, and a method emitted on one passed the whole corpus: the first mutation of ADR-0411 killed nothing. What distinguishes them is a module translated **alone** against a client that read another component first, which is the ordinary situation for a library and which `stale-component`'s sixth claim builds by hand. The row is that the *corpus* cannot, and that the gate is the only thing standing between a composed name and a coincidence | ADR-0411 |
| **Nothing here notices that a construct's own corpus is thin** | `tests/dialect/methods.pas` was written with the feature and exercises every receiver form its author thought of, and `procedure-coverage`, `line-coverage` and `kind-exhaustive` were all satisfied by it. Four defects were left, and each needed something the corpus did not have: two types in one translation with routines of one spelling (the enclosing implementation shadowed the receiver), a receiver that is not a bare name in a statement, a chain in a statement, and a field in a variant part. All four came out on the first day a *library* used the construct, which is §4a's rule -- and the register's point is that no gate here can ask whether a corpus enumerated a surface, because the corpus is what every gate measures against. **Two more came out of the fifteen libraries after it**, which is the same rule at a larger scale: an inherent implementation for a type produced from a schema was *accepted* until measuring `PasFile` asked what it would mean (ADR-0413), and a method could not name itself through a receiver in one spelling of three until a linked list wanted to (ADR-0415). Neither is reachable from a corpus written by the person who designed the construct | ADR-0315, ADR-0410, ADR-0412, ADR-0413, ADR-0415 |
| An implementation is reachable across a component and **nothing hides one** | AP 6.7.10.5 makes every implementation for an exported type selectable by every client, and there is no `only`, no `qualified` and no way to write one in an interface. So a module cannot have a method it does not publish, and adding one to a library type is as public a change as adding an export -- with nothing like `export-unique` watching it, a method not being an exported name | ADR-0411 |
...
| **`benchmark` is blind three ways** | A compiler slowed *uniformly* moves nothing, because what is committed is proportions; a stage made a fifth slower is under the threshold, which is about a third; and it abstains on CI and on aarch64 altogether, a stage share not being architecture-independent and the tolerances being margins over an idle machine — closing that needs an idle aarch64 machine to calibrate on. The milliseconds are recorded beside the machine that took them and read by a person | ADR-0270, ADR-0282 |
...
| The coverage gates are **ratchets**, and some of the five hold a number no ratchet can | `line`/`branch` ratchet one way by choice; `lib` and `lsp` fail both ways, being deterministic; `runtime`'s units that hold a wait with a deadline print and are not compared, three lines running only when a deadline beats a receive. **Three of them since ADR-0405** — the file half of the POSIX unit took `pasx_fd_ready` with it, and a poll with a timeout is the same race wherever it lives, so the count here follows a cut and is not a fact about the runtime | ADR-0104, ADR-0274, ADR-0350 – ADR-0354, ADR-0405 |
...
| **The runtime's error paths are a third of one file and no oracle can arrange them** | `pasrt_posix.c` is the low row of `runtime-coverage`: a failed bind, a spawn that cannot find its program. ADR-0405 moved the directory walk and the file information to `pasrt_file.c` and the shape went with them — a directory vanishing mid-walk is as unarrangeable there. What is missing is **fault injection**, and those lines are ones ASan, UBSan, LSan and TSan have looked at zero times | ADR-0261, ADR-0342, ADR-0351 |
...
| **Whether another gate's corpus is written into it has been asked of one gate and not of the rest** | `kind-exhaustive` read the compiler's three components and nothing else for its whole life, and the compiler could answer the same question about any Pascal program here — which is how `MenuKey` came to stop the shipped editor on two key kinds ADR-0396 had added, with every oracle green. ADR-0404 widened that one to five corpora. **The general question it raises is open**: `variant-check`, `predicate-kinds` and `ast-fields` are about the compiler's own AST and are not questions about another program, but `procedure-coverage`, `diagnostic-coverage` and `buffer-headroom` each name their corpus and none has been re-asked. The shape to look for is a gate whose corpus is a line of code rather than a list that grows with the tree | ADR-0145, ADR-0404 |
| **The clause inventory has no oracle of its own, in two directions** | It cannot tell it was given the wrong *document*: `extract.py` has a 50-heading floor and nothing else, so fed the Free Pascal reference as `iso10206.pdf` it extracts 190 "clauses", writes the inventory and exits 0, and every citation gate then checks against a bogus authority. And it cannot tell a clause was **dropped**: inventory and triage are checked against each other, so a clause the extractor missed and nobody triaged leaves both files agreeing and wrong. Only a reader holding the standard sees either | ADR-0105, ADR-0152, ADR-0164, ADR-0366 |
| **Nothing detects a harness whose diagnostics go nowhere** | `tls` had this for its whole life and it is fixed: `exec 3<>/dev/tcp/… 2>/dev/null` makes the redirection permanent for the shell, so from the first successful port probe — every ordinary run — fd 2 was `/dev/null` and every later diagnostic was swallowed; the gate failed correctly and printed nothing to read. ADR-0366's conversion has no such construct. **The live gap is the general one**, and the headline said otherwise until 2026-09-12: a gate that fails with an empty explanation looks exactly like one that fails with a full one, and nothing reads what a failing gate wrote | ADR-0264, ADR-0366 |
...
| **Raw mode is asserted in part, and three of its six claims still by nobody** | `ctest` has no terminal, so `tests/dialect/lib_term.pas` pins only the *negative* path: `IsTerminal` false, `TermSize` answering `errAbsent` rather than a plausible number, `EnterRaw` refusing and changing nothing. `tui-terminal` reaches the positive path for **three** of the six, because the editor it drives cannot work without them — a read answers on one keystroke, so canonical mode is cleared; `TermSize` answers the size the harness set with `TIOCSWINSZ`, so it reads a real window; and the program exits with the terminal usable, which exercises the restore. **What no case reaches**: that the flags cleared are exactly the right ones, that the settings put back are the settings *taken* rather than a plausible set, a window size that **changes**, and `EnterRaw`'s `errFull` arm, which needs a first `EnterRaw` to succeed and is reached in neither direction | ADR-0262, ADR-0381, ADR-0402 |
| **East_Asian_Width has no conformance file, and it is the one Unicode property here nobody else checks** | Unicode publishes the property and publishes no test for it, so regenerating AP 6.4.15.13's header and diffing it checks that the *generator* is stable and not that the table is right. **Mostly closed by ADR-0400**: `icu-width` compares this tree's reading of `EastAsianWidth.txt` and `UnicodeData.txt` against ICU's, over all 1 112 064 code points, exact agreement and no catalogue. What is left is that two transcriptions of one upstream file can agree and both be wrong, and that a machine whose ICU predates the pinned Unicode **abstains** — where the row is open again | ADR-0189, ADR-0395, ADR-0400 |
| **A terminal doing with these bytes what the sequences say is checked by nothing, and so is a terminal's `COLORTERM` telling the truth** | `tui-palette` holds the two colour *tables* — every pairing to an extreme against a colour and to a floor, and every declared role to being drawn by some session — and `tui-terminal` holds the **emission**, driving the real editor under a pseudo-terminal and requiring every run it writes to be one the model decided, at the same columns, with the same cells, in that role's colour, over both tables. It is not the binding ADR-0262 declined: the pseudo-terminal is Python's, as `lsp/run.py`'s pipe is. What is left is not checkable here at all — that a real terminal renders these sequences as they are meant, and that an emulator setting `COLORTERM` understands the twenty-four-bit form, there being no query a terminal lacking it answers safely. The cost of a terminal that lies is bounded by the fallback being held to the same floor | ADR-0389, ADR-0391, ADR-0393, ADR-0394, ADR-0402 |
| **Nothing finds a golden that pins a kernel's behaviour** | `lib_net_wait`'s pinned which of a buffered line and a socket's reply a round saw first, and passed on Linux for its whole life and on macOS twice before failing there (ADR-0368). `lib_net`'s then pinned *how many* writes a closed connection takes to be refused — two on Linux, more than a hundred on macOS, where a hundred writes of twenty-five bytes fit in a send buffer — and the fix for that had itself been a larger bet on the same number. Every gate here compares output byte for byte, so a schedule or a buffer size written into a golden is indistinguishable from a fact about the program until a platform disagrees. **Two instances, and both were found by a platform and not by a gate** | ADR-0205, ADR-0368 |
| **`runtime-nonposix` holds one bit per unit, and a second blocker hides behind the first** | A unit blocked for a new reason while still blocked for an old one reads `blocked` either way, and the *names* inside a header that is present are not catalogued the way `runtime-isoc` catalogues them. It bites one of the five units: `pasrt.c`, `pasrt_unicode.c` and `pasrt_file.c` compile, `pasrt_posix.c`'s blockers are headers and are catalogued exactly, and `pasrt_task.c` is blocked by a header refusing to declare a thread — with nothing to say whether a second waits behind it. **What would close it is `runtime-isoc`'s own mechanism**, shimming every catalogued gap and requiring the unit to compile, which today means stubbing five headers' worth of sockets, terminals and processes for a target no program here is built for. The toolchain flags are this row met early and dodged **four** times, the last being the gate's own `-std=c11` against the build's `-std=gnu11` (ADR-0383) | ADR-0161, ADR-0369, ADR-0380, ADR-0382, ADR-0383 |
...
| Nothing checks an `external` declaration against the function it names | Two components may each declare `external 'strerror'` and §6.13 is working as intended; whether they *mean* the same routine is this row. The call site is the whole of the ABI: LLVM does not check a direct call against the declaration under opaque pointers, so a wrong arity, type or function is undefined behaviour with no diagnostic. **Three properties are held and no more.** A scalar's *width* — every `int64` in an `external` catalogued as a C type 64 bits wide everywhere, which is a claim a person wrote (ADR-0364). One function's *arity* — `_setjmp`, the only foreign function the emitted module names, compared against each admitted target's own header, and the only declaration here judged by reading one (ADR-0371). And one symbol having one *signature* — a C function declared in more than one module under `lib/` or `lsp/` must agree by representation or carry a catalogued argument (ADR-0376). Everything else about every other declaration is unchecked | ADR-0121, ADR-0129, ADR-0147, ADR-0364 |
...
| **Nothing detects a harness that ignores a path, target or flag it is handed** | Four so far: `sanitize.py` and `AFTERSCHOOL_PASCAL_OPT`, `llc_check.py` and the target, `seed_current.py` and an absolute path, and `lib_coverage.py` running a case with none of the arguments and no `.in` the suite gives it — 70 statements reported unreached that the corpus had been covering all along (ADR-0378), which is the same shape from the other side: not a flag ignored but the convention that makes a case a case. `require-consistency` does this for `*_REQUIRE`; the same for `AFTERSCHOOL_PASCAL_*` needs a judgement about which harness should read which. **One variable of the family is gated in both directions** and is the pattern for the rest — `runner-seam` (ADR-0384), which holds that the two harnesses start the program through the runner, that neither starts the *toolchain* through it, and that with it unset nothing reaches the wrapper. The other five variables are unjudged | ADR-0330, ADR-0335, ADR-0345, ADR-0347, ADR-0378, ADR-0384 |
...
| A dialect scenario cannot reach **6.13.1 or 6.11** | Both are rules about linking components and a scenario asserts what a program printed; they stay in `pending.txt` and are held by `stale-component` and `tests/dialect/`. `tests/spec/run.py --coverage` prints the pair rather than this row. **6.13.2 left this row on 2026-09-12**: `Given the program-component` translates a module of its own and links it (ADR-0411), so a scenario is no longer one component — what is still out of reach is a *second translation of the same component*, which is what 6.13.1 and the staleness half of 6.13.2 are about | ADR-0135, ADR-0144, ADR-0245, ADR-0411 |
...
| **The two temporary-file conditions no harness can stage** | `TemporaryPath`'s exclusivity is pinned inside one process, and two programs started in the same second are argued rather than staged (`TemporaryDirectory` is `mkdtemp` and outside this). And an auxiliary file is an exclusive `fopen` in `/tmp` and then in `.`: the five lines that answer when neither will have it — the collision retry, the refused `remove`, the two giving-ups — are the five `runtime-coverage` reports uncovered, and no harness here can make `/tmp` refuse | ADR-0243, ADR-0363, ADR-0390 |
...
| **`model-drift` is three gaps, not one** | It is scoped to a **range**, so a sibling commit can satisfy it — right for a push, wrong for a commit, and a bisector finds an unexplained CodeGen change; a per-commit gate would train people to write trailers without reading them. Its **judgement** runs on CI only, over committed work, and `python3 tests/checks/model_drift.py origin/main HEAD` before a push is the substitute (`model-drift-base` holds the half that has broken). And its **CodeGen region runs to end of file**, so driver work below the banner trips it — four times so far, a comment counting — which is on purpose: write the trailer whenever a commit touches `compiler.pas` below the banner, and never narrow a safety gate while a build is red | ADR-0013, ADR-0153, ADR-0166, ADR-0167 |
...
| Most triage reasons are still a **title or nothing** | 44 of the 144 `structural` rows share one sentence — *introduces the subclauses below it* — and **145 of the 399** `testable` rows carry no reason at all, so a checked row and an unexamined one look alike. Re-measured 2026-09-12 from `triage.tsv` itself, the previous pair having been a count nothing re-runs. Both directions were swept once; a `shall` about something a program cannot exercise is visible only to a reader | ADR-0106, ADR-0200, ADR-0204 |
...
| **The predicate gates leave three things open** | `predicate-callers` claims no program in its table is *accepted*, not that every call site was exercised — several probes are refused by a rule that fires first. Between it and `predicate-kinds` sits a guard asking the **wrong question** with the right answer, `IsStringType` where *takes the string path* was meant, which was all three text-model defects; `--like` narrows that and judges nothing, and a guard testing a flag is outside all three. And a diagnostic arm placed **ahead** of a predicate silences the predicate's own test: one before the `Assignable` call masked it at its only site and 623 cases stayed green over a restored out-of-bounds write. Ask the predicate, then choose words inside the failure | ADR-0143, ADR-0146, ADR-0194, ADR-0198 |
...
| **A generic is translated in its client, and that costs three things** | Instantiated by two translations it is emitted into both, each under its own counter — duplication and not a wrong answer. And its body may call only what its clients can reach: a module's internal routine called from one is an undefined symbol in the client, naming a counter, and `PasContainer` exports two helpers no caller wants as the workaround. Both fixes are one linkage decision — a name shared across translations — which ADR-0411 has since made for a *method* and not for these. **The third cost was found on 2026-09-13 and is paid at the gate rather than in the compiler**: the emitted body carries its *own* file's line numbers into the client's IR, so `lib-coverage` counted 50 statements of `PasContainer`'s across four modules as the clients' own, reported 18 of them uncovered in a module that never runs them, and moved for edits that touched no statement — three times in one day, twice for a comment. ADR-0416 attributes a hit to the routine whose marker opened it; the *language* property is unchanged and a second gate wanting the same attribution would have to repeat it | ADR-0211, ADR-0212, ADR-0416 |
...
| **A catalogued partial case is only as good as its argument, and nothing re-reads the argument when the enumeration grows** | `kind-exhaustive` compares a site's `N of M` against `partial_cases.txt` in both directions, so a numerator or a denominator that moves is caught. What it cannot read is the **reason** beside the row: `DumpTypeExpr`'s is argued as *each names its own third*, and adding a type-denoter kind falsified that sentence while 15 still equalled 15, stopping the compiler on `--dump-ast` over any program containing a `dyn` (ADR-0408). `procedure-coverage` caught it, by ADR-0269's rule that the compiler must *survive* every invocation — a second gate holding the first one's blind spot, which is why that rule exists. What would close it is a row saying which constants a subset is *closed over* rather than how many, and that is a catalogue of a different shape | ADR-0145, ADR-0269, ADR-0408 |
| **A trait-keyed call records no use, so no editor can find a trait routine's definition** | Sema records a use where it *resolves* one (ADR-0111, ADR-0230), and `DispatchTrait` is the one resolution that does not: neither arm calls `NoteUse`, so `--dump-uses` reports nothing for a call to a trait routine and `definition`, `references` and `rename` all miss it. **No oracle here can see it** — `--dump-uses` has no catalogue of what it should contain, and a use that is absent looks exactly like a name nobody wrote. Found by asking the dump while adding the procedure half (ADR-0407), and pre-existing on both sides, which is why that arm was written to match rather than fixed alone | ADR-0111, ADR-0340, ADR-0407 |
| **Three predicates decide what a function-designator is, and nothing compares them** | `IsCallValue`, `IsDesignator` and `CalledSym` answer *is this a call*, *is this a variable* and *which routine* about one node, and every rule asks one of them by name (ADR-0179). They must agree: a spelling that is a call to the first and a designator to the second reaches a `var` parameter, which is how AP 6.7.10.4's method-designator passed a value where the callee wanted an address (ADR-0410). **`kind-exhaustive` cannot see it** — the disagreement was inside an arm it already counted. Declined because the answer is defined only for the handful of node kinds that can be either, so the catalogue would be a list of the cases somebody thought of | ADR-0179, ADR-0410 |
| **The one indirect call the emitter makes has no lowering proof, and `verify/` is the wrong instrument for it** | AP 6.7.11's trait object loads a code address and a static link out of a table and calls them (ADR-0409). The property — *the call reaches the routine the table names* — is the emitted code written a second time, which is the nil-check's shape ADR-0013 says dilutes *no known gaps*, so no rule was written, deliberately. What holds it instead is a behavioural case under `valgrind-corpus` and `sanitizers`, plus the heap balance. **What none of those sees is a table built wrongly in a way every entry agrees about** — a self-consistent wrong slot order; `tests/dialect/dyn.pas` declares two of its four implementations out of the trait's order so the commonest such mistake is caught, and that is a case rather than a proof | ADR-0013, ADR-0409 |
| **Nothing checks that a harness works only inside the directory it made** | Every writing harness calls `mktemp -d` or `mkdtemp` and asks for a port; a grep proves the directory exists and not that nothing is written outside it — `format_check.py` shipped writing a fixed path with every gate green. Declined rather than written as a proof that is not one. It is **one thing smaller** since ADR-0406, which started the program under test in that directory too: what it does not cover is a *harness* writing outside its own. Its closing sentence used to read *every local parallel run now exercises it*, and that sentence was half the defect — the program under test ran in the invoker's directory, a local parallel run exercised it perfectly, and CI lost the race on three jobs | ADR-0281, ADR-0406 |
...
| **Only two harnesses can run a program for another target** | `AFTERSCHOOL_PASCAL_RUNNER` is read by `tests/run_test.py` and `selfhost/irtest.py` and by nothing else (ADR-0384), so `sanitize.py`'s four modes, `target32.py`, `lib_coverage.py` and every other sweep executes what it built. That is deliberate — each builds a particular *native* configuration and a program for another target is not the thing it instrumented — and the consequence is that setting the variable for a whole `ctest` run makes those sweeps fail to exec rather than answer. Failing loudly is the right failure and it is still not a check: nothing refuses the variable where it cannot mean anything | ADR-0384 |
| **The wasm catalogue is one engine's, and an older one cannot take the measurement** | wasmedge 0.16 answers a null handle where 0.17 answers a stream for `fopen("x","w")` under the same `--dir /:/`, so two corpus programs that open a scratch file beside themselves fail on the older engine and pass on the newer. `wasm32` probes for it in C and **abstains** rather than reporting two defects that are not there (ADR-0385's rule, met a second time). What stays unchecked is the other direction: nothing here notices if a *future* engine changes behaviour again in a way the probe does not ask about, and the committed numbers are testing's wasmedge 0.17 | ADR-0385, ADR-0390 |
| **A number quoted from a gate that runs only in a job of its own is checked nowhere** | `quoted-numbers` reads what the gates last printed, from the ctest log, after the suite in the `test` job. A gate needing a toolchain that job has not got skips there and answers in a job of its own — `wasm32` and `runtime-nonposix` — so its rows come back *unchecked* every time and nobody reads that line. Found by hand on 2026-09-12: three documents said **563 of 598** programs run as WebAssembly where the gate had been answering **570 of 605**, and the rows watching those exact sentences had never once been evaluated. The fix is to run the check in those jobs against their own output; it is not built because its evidence is a *ctest* log and those gates are invoked directly | ADR-0379, ADR-0385 |
| **`wasm32` support requires an LLVM that names `i128` for the target** | The module states `i128:128` and an older LLVM does not, so it aligns an i256 to 8 where this compiler says 16 — and clang *overrides* the module's line with its own for the target it is given (ADR-0156), so a set inside a record is laid out two ways, which is ADR-0028's defect. **No longer unwitnessed**: `tests/sets_records.pas`, the case that decision exists for, was the single failure when the job ran on trixie. `target-layout` and `wasm32` both abstain where this clang and this module disagree about a modelled field, and the CI image is `debian:testing` so the measurement can be taken. What stays unchecked is the requirement itself — a *reader* on an older LLVM gets a skip and a printed reason, and nothing fails | ADR-0028, ADR-0156, ADR-0383, ADR-0385 |
