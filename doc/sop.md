...
| **A row's colour is the first cell's, and nothing checks that a row is one colour** | `PutRow` takes `scr.role[r][1]` and paints the whole row with it (ADR-0389). That is true of every row this editor draws today -- text, status, hint, message, prompt are each uniform -- and it is an assumption rather than a claim: a row of mixed roles would be painted wrongly with no diagnostic, and the session goldens hold the *roles* and not what the shell made of them, so they cannot see it either. It becomes false the moment a panel overlaps a document row, which is the next structural step; what has to change then is `PutRow` splitting a row into runs, and the comment at the call site says so | ADR-0389 |
