...
| A **defining** occurrence answers `null`, where an editor answers the declaration itself | ADR-0246's dump reports *applied* occurrences, so a position on a `var` line, a procedure heading, a type definition or an export-part has no `use` line over it and go-to-definition there gives nothing. It is uniform and it is not what an editor's users expect: every other language server answers a declaration with itself. `lsp/sessions/definition_across.jsonl` pins the current behaviour so that changing it is a decision rather than a drift. What would close it is the compiler reporting every defining-point as an occurrence of itself, which changes what a `use` line means and adds a line per declaration to a dump that is already a line per name — or the server falling back to `--dump-symbols`, which it already runs for the outline, at the cost of a second compilation per request | ADR-0248, ADR-0246, ADR-0239 |
