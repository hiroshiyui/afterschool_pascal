...
| Nothing runs a program with **no writable temporary directory** | An auxiliary file is an exclusive `fopen` in `/tmp` and then in `.` (ADR-0388); the five lines that answer when neither will have it — the collision retry, the refused `remove`, and the two giving-ups — are the five `runtime-coverage` reports as newly uncovered, and no harness here can make `/tmp` refuse | ADR-0388 |
