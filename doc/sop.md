...
| **Raw mode itself is asserted by nothing, and the only oracle is a person using a program** | `ctest` has no terminal: `tests/dialect/lib_term.pas` runs with every descriptor a regular file, so it pins the *negative* path — `IsTerminal` false, `TermSize` answering `errAbsent` rather than a plausible number, `EnterRaw` refusing and changing nothing. What no case reaches: that the flags cleared are the right ones, that a read then answers on one keystroke, that the settings put back are the settings taken, the `atexit` restore, a real window size that *changes*, and `EnterRaw`'s `errFull` arm, which needs a first `EnterRaw` to succeed and so is reached in **neither** direction. All six were checked once by hand under a pseudo-terminal and by nothing since. **ADR-0262 promised this row and it was never written**, which is the register's own decay shape found from the other side; ADR-0381 makes it matter more, an editor being a program that lives in raw mode. Closing it needs a PTY binding, which ADR-0262 declined on the grounds that a case needing one becomes a test of that binding — the screen *content* is held instead, by rendering to a buffer the harness can diff as text | ADR-0262, ADR-0381 |
| **East_Asian_Width has no conformance file, and it is the one Unicode property here nobody else checks** | `unicode-conformance` is this tree's only oracle written by neither this processor's author nor its specification's, and it earns that by running Unicode's own `NormalizationTest.txt` and `GraphemeBreakTest.txt`. AP 6.4.15.13's table has no such file: Unicode publishes East_Asian_Width and publishes no test for it. What stands behind `pas_u_width` is that the transcription is simple, that the generator asserts the five blocks whose unassigned code points the file's *header* gives the value W really do come out W in its rows, and seven answers in `tests/dialect/lib_unicode.pas`. Regenerating the header and diffing it is a check that the *generator* is stable, not that the table is right. **Narrowed by ADR-0398 and ADR-0399**, which put the C library's own table beside it: two independent readings of one database now have to agree except where this language decided otherwise, and what is catalogued is a **cause** and not a code point — four of them, three read off the break table the header already carries. The first version enumerated ranges, passed where it was written and failed on two other platforms within the hour, which is ADR-0399's finding: every machine has a libc and they differ, so what can be written down is the *reason* two implementations are allowed to. **Three things stay uncovered and ADR-0399 measured all three.** Both readings could transcribe the same upstream change. Exact agreement is impossible to require — two C libraries are built from different Unicode versions, ubuntu:24.04's predating the release that made U+2630..U+2637 Wide — so what is claimed is that divergence stays under 500 code points, a bound set by measurement: the smallest systematic error here is 2307 code points and a version skew is tens. Under that bound an error in one code point is invisible, which `unicode-conformance` covers by regenerating and diffing. And an error *inside* a class the clause declares open — zero-width, spacing marks, Ambiguous — is invisible to both, because a library disagreeing there is precisely what the causes permit: if the generator stopped zeroing marks, every disagreement would be explained and this would pass | ADR-0189, ADR-0395, ADR-0398, ADR-0399 |
| **What the shell *emits* for a role is asserted by nothing, and the table it emits from now is** | ADR-0389 split a cell's **role** from its colour so a golden could hold the first without the second, and ADR-0391 had `PutRow` split a row into runs and write one `SetColour` per run. Both records promised a row here and neither wrote one, which is the register's own decay shape a third time. ADR-0393 closes the half that could be closed: `tui-palette` reads `RoleColour` and holds every pairing to an extreme against a colour and to a floor, and holds every declared role to being drawn by some session — that second claim found `crPrompt` declared, coloured, lettered and drawn on no screen for the whole life of ADR-0392, with all fifteen sessions green. What is still uncovered is the **emission**: that the runs written match the role plane, that the sequences are the ones `PasTerm` means, and that a terminal does with the eight ANSI colours what xterm does. The first two were checked once by hand under a pseudo-terminal; the third is not checkable here and is why the rule is structural and the ratio only evidence. ADR-0394 adds a second table for a terminal that says it understands twenty-four-bit colour, and **that a terminal saying so is telling the truth is checked by nothing** — `COLORTERM` is what an emulator sets and there is no query a terminal lacking the form answers safely, so a terminal that lies gets a scheme it cannot render. The cost is bounded by the fallback being held to the same floor | ADR-0389, ADR-0391, ADR-0393, ADR-0394 |
