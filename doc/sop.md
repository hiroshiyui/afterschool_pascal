...
- **B1. Cite the clause**, in the code comment and the commit message. A rule
  ...
  - **The number is checked to exist** by `clause-citations` (ADR-0164), which
    is the half that *can* be mechanical: over the citations in the tree — 10 415 across 1671 files as this is written, and the gate prints the pair — it asks whether
...
| **An array's index-type may span one fewer value than it should** | The lowering subtracts `i - lo` unchecked, so the bound must guarantee `hi - lo` is a value of the integer type — `-maxint..maxint`, hence `hi - lo <= maxint`. Sema refuses at `hi - lo >= maxint` and `verify/iso.py`'s `index_span_is_representable` says `< maxint`, **and the two agree**, so `verify/` proves the lowering under a precondition stricter than its own docstring's reason and no gate can see the difference. `array [0..maxint] of char` is refused at 2 147 483 648 bytes while a 2 400 000 000-byte record is accepted, so it is not a capacity limit and ISO 7185 §1.2 a) does not cover it. Relaxing the predicate to `<=` leaves **all 48 rules proved** — measured, not argued. Not fixed: it needs `TypeLength` widened to int64, the bound restated, the model moved in the same commit (ADR-0013), and cases; and it is a language change, so it takes its own record | ADR-0013, ADR-0287, ADR-0288 |
