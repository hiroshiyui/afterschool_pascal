...
| **`selfhost/irtest.sh`**, stage 2 = stage 3 | that the compiler is a fixed point under self-application | a bug **stable** under self-application; both stages come from one binary |
...
| **`selfhost/producttest.sh`** | that the artefact built is the one described | anything its checks do not ask |
...
- **C1. `PAS_FILE_SIZE` and `fileSize` must agree** if `struct pas_file` grew;
  the two files cannot include one another and `irtest.sh` checks it.
...
| **`lib-coverage` cannot measure a case that takes a program-parameter** | It runs every case with no arguments, so `lib_fs.pas` stops at its first statement under the sweep. `lib_fs_tempdir.pas` is the parameterless workaround; the fix is the sweep reading `run_test.sh`'s argument convention | ADR-0350, ADR-0363 |
...
| `-O0` and `-O2` are each run, never **compared** | A case where the two differ and both look plausible passes twice; only `--crosscheck` compares them, over its own program. `irtest.sh` links at `-O0` unconditionally, which is how ADR-0220 was found, and must not be leaned on | §6, ADR-0220 |
...
| **Two library modules are exercised by one harness and no case** | `PasTls` and `PasHttps` are reached by `tls.sh` alone, so `lib-coverage` reports them zero and says why; driving `tls.sh` from it would make the number move with a machine's packages | ADR-0264, ADR-0350 |
...
| `coverage.py` sees the sources, not the harnesses that build their own compiler | What `irtest.sh`, `producttest.sh` and `verify.py` drive is invisible; the *flags* half is closed by sweeping `--dump-all` and `--coverage` over every source | ADR-0103 |
...
| The diverse-double-compiling window **closed without anything noticing** | `seed/ddc.sh` answered the seed's provenance once; `v0.1.0` cannot read three program-components, so there is no longer a way to check it from outside | ADR-0085, ADR-0233 |
...
| **Nothing detects a harness that ignores a path, target or flag it is handed** | Three in a week: `sanitize.sh` and `AFTERSCHOOL_PASCAL_OPT`, `llc_check.py` and the target, `seed_current.sh` and an absolute path. `require-consistency` does this for `*_REQUIRE`; the same for `AFTERSCHOOL_PASCAL_*` needs a judgement about which harness should read which | ADR-0330, ADR-0335, ADR-0345, ADR-0347 |
...
| A **value transcribed from a C header** is checked for one library and no other | `tls.sh` checks the six OpenSSL numbers; nothing derives the list, and `PasProcess.ClocksPerSec` carries the same hazard with no gate. An annotation the compiler could report, as `@cstruct` is, was not built | ADR-0185, ADR-0264 |
