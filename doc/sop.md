...
| **Two gates flaked once each on 2026-09-06, and the second is not a short read** | `fpc-differential` failed in a full run and passed on rerun and on a second full run. And a coverage sweep saw `tests/extended/lib_strings.pas` fail with *unexpected character '}'* at 10:53 — the closing brace of its leading comment, so the `{` on line 1 had not opened one. **What was ruled out**, so nobody does it twice: the reported position is *correct* for that file, which means the reader saw every newline and the file was not truncated — a short read would have shifted the count. And 400 parallel compilations of that source with that compiler produced no error at all, so it is not a function of load on the reader. What is left is nondeterminism inside `pascalc`, of which the likeliest shape is a **read of uninitialised memory** — `ast-fields` (ADR-0222) exists because §6.5.3.3 gives a variant record no member initialisers, and the sweep that saw it was running against a differently laid-out runtime. **Nothing here can see that class**: ADR-0342 established that ASan does not instrument compiled Pascal, and uninitialised reads want MemorySanitizer, which needs everything including libc instrumented. Valgrind needs no instrumentation and would see it; it is not installed here, and that is the cheapest next step if this recurs. Nothing catches an intermittent either way — every gate is a single run, and a defect with a rate rather than a trigger passes almost all of them | ADR-0222, ADR-0281, ADR-0342, ADR-0351 |
| The coverage gates are **ratchets**, not allowlists, and there are now **five** | none can fail in both directions, so a line that becomes covered says nothing and the uncovered ones carry no argument between them; the per-procedure or per-module breakdown is what makes a regression nameable. It was one file and is now `line_coverage.txt`, `branch_coverage.txt` (ADR-0104, ADR-0274), `lib_coverage.txt` (ADR-0350), `runtime_coverage.txt` (ADR-0351) and `lsp_coverage.txt` (ADR-0352) — four of them landed on one day, which is when this row stopped being about one gate. **The weakness compounds**: five one-directional numbers are five nobody has to justify, where `uncovered_procedures.txt` and `heap_balance.txt` demand an argument per row and fail when one stops being true. A per-line argument is not writable at this scale and that is the whole of why; it is a cost accepted five times rather than a thing nobody noticed. **Two of the five carry a second number that is ratcheted the same way and is not coverage** — `lsp_coverage.txt`'s `ambiguous`, the server statements a generic instantiation makes unattributable, and `runtime_coverage.txt`'s branch figure, reported and deliberately not gated | ADR-0104, ADR-0274, ADR-0350, ADR-0351, ADR-0352 |
| **Two library modules are exercised by one harness and no case** | `PasTls` and `PasHttps` are reached by `tests/checks/tls.sh` alone, which is a gate and not a corpus case, so `lib-coverage` reports them 0 of 168 statements and says in its own header that this is not what uncovered means. They *are* run on every push — `TLS_REQUIRE` and a CI job refuse to let that gate pass by skipping (ADR-0264, ADR-0330) — so what this row records is the **shape** and not a hole: every other module has a `tests/dialect/lib_*.pas` driving it, and a reader comparing per-module numbers must know why those two read zero. Driving `tls.sh` from `lib-coverage` was rejected because the gate skips without libssl and the number would then move with a machine's packages, which is ADR-0346's lesson the same day | ADR-0264, ADR-0350 |
| **The runtime's error paths are 68.6% of one file and no oracle can arrange them** | `runtime-coverage` (ADR-0351) answers 84.3% over `runtime/*.c` and `runtime/pasrt_posix.c` is the low row at 68.6%, its name being the reason: the sockets, the directory walk, the process and the clock, whose failure arms ask for a bind that fails or a directory that vanishes mid-walk. A corpus running on a working machine cannot produce either, so what is missing is **fault injection** and not a case somebody forgot to write — the same shape as `fuzz`'s half of the sanitizer gap, one directory over. It matters more than a coverage number usually does: ADR-0342 established that AddressSanitizer never instruments compiled Pascal, so those lines are not merely unmeasured, they are lines ASan, UBSan, LSan and TSan have looked at zero times | ADR-0261, ADR-0342, ADR-0351 |
| **`tools/pascalcc` is 688 lines with no coverage instrument of any kind** | it is shell, so `--coverage` cannot reach it and nothing here can say a branch of it ran. It grew ~200 lines with ADR-0348's project subcommands and now holds an argument parser, a TOML reader with ten keys and four refusal paths, and four subcommands. `new-project` and `producttest` exercise it well by hand — every config key is asserted, two of them only after a review found them parsed and dropped — but *well by hand* is what every corpus in `doc/history.md` was opened by. ADR-0348 already leaves open whether the generator belongs in a shell driver at all, and this is the measurement half of that question | ADR-0348, ADR-0350 |
...
| **Nothing detects a harness that ignores an environment variable it is handed** | ADR-0335 is the instance: `sanitize.sh` compiled 383 corpus programs at a level it chose itself and read `AFTERSCHOOL_PASCAL_OPT` not at all, so `AFTERSCHOOL_PASCAL_OPT=-O0 ctest` reported the whole suite green at `-O0` with two of its cases at `-O1` — and CI's `unoptimised` job made that claim on every push. `require-consistency` (ADR-0330) does exactly this comparison for `*_REQUIRE`, in both directions, and the same shape for `AFTERSCHOOL_PASCAL_*` would have to know which harness *should* read which variable, which is a judgement rather than a comparison: `foreign-reserved` has no business honouring an optimisation level and `target_layout.py` none honouring a runtime path. What is available cheaply is the negative half — a variable a harness sets in a subprocess environment and no harness reads — and it is not built. **It has now happened three times, twice in one day**: `llc_check.sh` ignored `AFTERSCHOOL_PASCAL_TARGET`, so the `second-backend on aarch64` job compared x86-64 modules on an ARM machine and had never passed since the day it was added (ADR-0345); and `seed_current.sh` handed the compiler an **absolute** source path, which ADR-0293 puts into the emitted module, so it compared a directory and could pass only where the seed was made (ADR-0347). All three are a harness answering, in good faith, about something other than what it was asked — and the general shape is wider than an environment variable: it is a harness passing a path, a target or a flag that is right where it was written and wrong where it runs | ADR-0330, ADR-0334, ADR-0335, ADR-0345, ADR-0347 |
| **Every gate but `target32` and the two sanitizer modes is run at one optimisation level** | ADR-0334 found what that costs: `tests/dialect/int64_foreign.pas` declared C's `labs` as taking an `int64`, a wrong ABI on every ILP32 target, and `target32` swept it 570 sources at a time for two weeks and passed — at `-O2` the optimiser folds `labs` of a constant away before the ABI matters, and at `-O0` it answers `-3028092405585415680`. **The combination was run by no job**: `thirty-two-bit` took the default `-O2`, and the `unoptimised` job has no 32-bit libc, so `target32` skips inside it and ctest reads the skip as success. Two jobs, two axes, and the cell where they cross was empty. `thirty-two-bit` now runs the gate twice. What is **not** closed is the general question: `sanitizers`, `tls`, `fpc-differential`, `second-backend` and `llc-second-backend` each answer at one level, and the argument that the `unoptimised` job covers `-O0` holds only for a gate that does not skip in it — every gate in that list skips without a package that job does not install. **The rest of this row was measured on 2026-09-05 and is now closed rather than open** (ADR-0335): `tls` drives its behavioural half through `run_test.sh` and already honours the variable, `llc-second-backend` runs *both* levels internally and always has, and `fpc-differential`, `target-sizes` and `unicode-conformance` ask questions no optimiser can change — a disagreement about a printed value, two struct sizes, and the Unicode database. The two that did not honour it were the sanitizer modes, and they do now; all three levels are clean, which is a number and not an expectation | ADR-0325, ADR-0328, ADR-0334, ADR-0335 |
...
| ~~**AddressSanitizer does not see compiled Pascal at all**~~ — the class is covered since 2026-09-07 (ADR-0353), the row kept for what is still true | The emitted IR carries no `sanitize_address` attribute, and clang's pass instruments only functions that do — so `AFTERSCHOOL_PASCAL_CFLAGS=-fsanitize=address` reaches the compilation of the `.ll` and changes nothing about the program's own loads and stores. A plain `new(p); q := p; dispose(p); q^ := 5` runs clean and prints 5 under a fully ASan-linked binary. `sanitizers` is honestly described — it asks whether the **runtime's own C** survives the suite, and it does that — but every argument of the form *ASan reports nothing* made about a **program's** behaviour is empty, and the row above makes one. The same holds of `thread-sanitizer` and of every TSan run this project has done by hand. **The fix is one attribute and was measured**: with `sanitize_address` on the emitted functions, that program reports the use-after-free with a stack trace. It is not taken, being a change to what every compilation emits and one that would redden the gate over whatever 383 corpus programs turn out to contain — which is a decision, and it has still never been put. **What changed is that it no longer has to be**: `valgrind-corpus` instruments nothing, reads the binary, reports that probe as an invalid write and an invalid read, and finds the corpus clean at 377 of 377 (ADR-0353). So the *class* is covered and the *speed* is not — ASan is orders of magnitude faster and could run where 170 seconds cannot. Every sentence above about what the four sanitizers watch remains exactly true of them | ADR-0261, ADR-0327, ADR-0342, ADR-0353 |
| **Four of AP 6.4.14.7's borrow residues stand, and the clause no longer says otherwise** | A 2026-09-06 audit wrote five shapes where a with-binding or a borrow survives a release reached through a **second name**: a with-body calling a helper that disposes the owner (`with o^ do Kill(o)`), the owner reached under two names one activation down, a *function actual* in the same call that releases it, and a nested callee reaching the owner non-locally. Each compiles, runs, writes to freed storage and exits 0. The fifth — `with o^ do s := take(o)` — is closed (ADR-0342): a move releases nothing, so none of the clause's three release points saw it. The other four are exactly the class ADR-0319 costed and declined, and what changed is that AP 6.4.14.7 NOTE 6 no longer claims *the two together leave no residue* | ADR-0319, ADR-0342 |
| **Two readings of the memory model are unsettled and nothing decides them** | AP 6.4.14.7 a) says an actual "denotes an entire-variable whose type contains that owned-pointer-type" without saying it must be the **same** entire-variable that owns the first; NOTE 2 supplies that, and a NOTE is not normative. And AP 6.4.17.2 says a task shall not be a function result while AP 6.4.12.6 admits a factory and AP 6.4.17.2 opens by adopting everything 6.4.12 says — the compiler admits `function F: task`. Both are UNSETTLED verdicts, so neither takes a scenario: a suite asserting one of two defensible readings launders a coin-flip into a citation | ADR-0342 |
