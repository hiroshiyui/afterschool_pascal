...
| A **field selection** is answered by no dump, where a discriminant is | ADR-0246's `--dump-uses` reports every applied occurrence that resolves to a *symbol*, and a record field is a `fieldPtr`: it has no defining-point of that shape, so `r.x` produces no line and go-to-definition on a field answers `null`. A schema's discriminant looks identical in the source and **is** reported, being a symbol — so the asymmetry is visible in `tests/dumps/uses.dump` and nothing explains it there. What closes it is giving a field a position and a route from an occurrence to it, which is two integers on `fieldPtr` and one call site; it is registered rather than done because a second shape of `use` line — one whose target is not a symbol — is a decision about the format and nothing has asked for it twice | ADR-0246, ADR-0239 |
| An **interface** has a name and no position | `ifaceRec` holds a name, an owner and its constituents, and never where the `export` clause was written — so §6.11.3's `M.x` hovers on `M` and jumps nowhere. Registered because it is the one part of ADR-0246's answer that is missing a *fact* rather than a decision: the two spans, the narrowest-wins rule and the kind word are all in place and the target is 0. Three integers on that record and one assignment where an interface is registered would close it, and there is one caller | ADR-0246, ADR-0079 |
| A name inside a **schema's body** resolves in no file the dump can name | §6.4.7 produces a type by re-resolving the schema's body once per tuple, **where the type is written** — so `curFile` is the writer's while the body's line and column belong to wherever the schema was declared, which for `string(80)` is `lib/`. ADR-0246's dump excludes productions for exactly that reason and the cost is that `cap` in `array [1..cap]` is reported nowhere, being resolved nowhere else. AP 6.7.3.10's instantiation is the same shape and *is* reported, because it puts `curFile` to the generic's own file; a schema production could do the same and does not, the file it would name being on the schema's symbol rather than in hand at the production. Nothing detects this: the missing lines look exactly like a source that never used the name | ADR-0246, ADR-0209 |
