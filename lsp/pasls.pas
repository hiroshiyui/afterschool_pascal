  ...
  for i := 1 to SVecLen(lines) do begin
    ...
    if SymField(text, 1) = 'use' then
      if (ValueOr(ParseInt(SymField(text, 5)), -1) = keyFile)
         ...
         and (ValueOr(ParseInt(SymField(text, 7)), 0) = s.declCol) then
        AddRef(s, inFile, ValueOr(ParseInt(SymField(text, 2)), 0),
               ValueOr(ParseInt(SymField(text, 3)), 0),
               ValueOr(ParseInt(SymField(text, 4)), 0))
  end
end;
