...
{ ApFront -- the lexer, the parser and the AST, and Sema.

  The second of the three program-components (ADR-0233). It imports ApTypes and
  is imported by the program, so it is the component that makes every build
  translate a module that imports another -- the shape the corpus was missing
  when the defect of ADR-0212 and ADR-0216 got through.

  A port of src/lexer.cpp, src/parser.cpp, src/ast.h, src/sema.cpp and
  src/type.h, from a second implementation that was deleted with the
  conformance surface it was frozen at (ADR-0232). Two shapes here are forced
  ...
  * The parser's one exception (ParseAbort) becomes a flag. Pascal has no
  ...
  What it exports is the whole of what a code generator needs from a front end:
  the three stage entry points, the two dumps, and the handful of queries
  CodeGen asks about a tree Sema has already annotated. How a name is resolved
  and how a type is checked stay inside -- 6.11.1 puts the interface in the
  module-heading, so what is not written there cannot be reached.

  Sema's contract with the code generator is the reason that interface can be
  small: every node's `ntype` is non-null and every nkVar's symbol is resolved
  when this component is done, so CodeGen never inspects a name and never
  re-derives a type (ADR-0008). }
...
var
  ...
  realType, boolType, charType, voidType, nilType, textType: typePtr;
      ...
      nkDeref: begin
        ...
        { ADR-0123: `o^` on an optional is the value, and it is the only way to
          ...
          { 6.4.4 gives a pointer-type one nil-value and a set of
            ...
            the nil-value is not an identifying-value, it does not identify a
            variable." So `nil^` is not a dereference of something that is not
            a pointer -- it is a pointer with nothing on the other end, and the
            general message would be naming the wrong rule. Only reachable as a
            written program since a constant may be `nil`. }
          if IsNil(b) then begin
            ErrorAt(e^.line, e^.col);
            writeln('nil identifies no variable, so it cannot be dereferenced')
          end
    ...
    case s^.kind of
      ...
      skConst:
        ...
        { A pointer constant is `nil` and can be nothing else: 6.4.4 gives the
          token one value, and the only pointer a constant-expression can name
          is that one. So the word is printed rather than intVal, which is a
          field the fold never wrote -- the same care the memory case above
          takes, and for the same reason. }
        else if IsPointer(s^.stype) then write('const nil')
